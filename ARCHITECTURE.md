# Eburon Hub: Developer and Architecture Guide

> This document is the canonical reference for anyone (human or agent) working on the
> Eburon Hub. It covers the exact current state of the code, every design decision and
...
The Eburon Hub is a lightweight web application that gives you a browser-based
interface to the Beatrice agent that is functionally equivalent to the CLI. It is modeled on
...
Eburon Hub-level chrome is intentionally consolidated: the sidebar has no dedicated brand header.
Instead, the footer exposes a single "Eburon Hub" launch button that opens one tabbed
...
## 2. File Inventory
    ...
    api/
      ...
      profiles.py          Profile state management, eburon_cli wrapper (~246 lines)
      ...
      streaming.py         SSE engine, run_agent, cancel, EBURON_HOME save/restore (~236 lines)
    ...
    tests/
      conftest.py          Isolated test server (port 8788, separate EBURON_HOME) (~240 lines)
...
- The venv has all Beatrice agent dependencies (run_agent, tools/*, cron/*)
...
- The server imports Eburon Hub modules via sys.path.insert(0, parent_dir)

Environment variables controlling behavior:

    EBURON_WEBUI_HOST              Bind address (default: 127.0.0.1)
    EBURON_WEBUI_PORT              Port (default: 8787)
    EBURON_WEBUI_DEFAULT_WORKSPACE Default workspace path for new sessions
    EBURON_WEBUI_STATE_DIR         Where sessions/ folder lives
    EBURON_CONFIG_PATH             Path to ~/.eburon/config.yaml
    EBURON_WEBUI_DEFAULT_MODEL     Default LLM model string
    EBURON_WEBUI_PASSWORD          Optional: enable password auth (off by default)
    EBURON_HOME                    Base directory for Eburon Hub state (~/.eburon by default)

Test isolation environment variables (set by conftest.py):

    EBURON_WEBUI_PORT=8788                           Isolated test port
    EBURON_WEBUI_STATE_DIR=~/.eburon/webui-mvp-test  Isolated test state
    EBURON_WEBUI_DEFAULT_WORKSPACE=.../test-workspace Isolated test workspace
...
Per-request environment variables (set by chat handler, restored after):
    ...
    EBURON_EXEC_ASK      Set to "1" to enable approval gate for dangerous commands.
    EBURON_SESSION_KEY   Set to session_id. The approval tool keys pending entries
                         by this value, enabling per-session approval state.
    EBURON_HOME          Set to the active profile's directory before running agent.
...
2. Sets TERMINAL_CWD, EBURON_EXEC_ASK, EBURON_SESSION_KEY env vars
...
The approval system uses the existing Eburon Hub gateway module at tools/approval.py.
...
Three-panel layout (in static/index.html):
    ...
    <aside class="sidebar">    Left panel: session list, nav tabs, sidebar-footer Eburon Hub trigger
...
Boot IIFE:
    localStorage key 'eburon-webui-session' stores last session_id
...
9.  setBusy(true), setStatus('Eburon Hub is thinking...')
...
The api/ modules in turn import Eburon Hub internals:
...
On startup, server.py reads ~/.eburon/config.yaml:
...
Root cause: TERMINAL_CWD, EBURON_EXEC_ASK, EBURON_SESSION_KEY are set via os.environ
...
with separate EBURON_HOME, wiped per run. Production data never touched.
...
Endpoint that calls Eburon Hub Python modules:
...
## 12. Common Debugging Commands
    ...
    ls -lt ~/.eburon/webui-mvp/sessions/
    cat ~/.eburon/webui-mvp/sessions/SESSION_ID.json | python3 -m json.tool
    ...
    ls ~/.eburon/webui-mvp/sessions/ | xargs -I{} python3 -c "
    import json, sys
    d = json.load(open('~/.eburon/webui-mvp/sessions/{}'))
...
Decision: EBURON_EXEC_ASK and EBURON_SESSION_KEY passed via os.environ.
...
## 14. Version History
    ...
    v0.12  Sprint 10 (March 31, 2026):
            ...
            Post-sprint fixes: SSE cancel event breaks loop, Cancel button always hidden on setBusy(false),
              ...
              Session.__init__ **kwargs forward-compat, test cron isolation via EBURON_HOME,
...
This section is specifically for agents (Eburon Hub instances, subagents, Codex, etc.) that
...
eburon-webui-mvp skill. Key ones:
...
~/.eburon/memories/), renders both as markdown via renderMd() with timestamps.
...
The symlink means all existing import paths (sys.path.insert for beatrice-agent modules)
continue working unchanged. start.sh updated to reference new canonical path.

Safe from: git pull, git reset --hard, git stash on beatrice-agent repo.
...
WORKSPACES_FILE = ~/.eburon/webui-mvp/workspaces.json
LAST_WORKSPACE_FILE = ~/.eburon/webui-mvp/last_workspace.txt
