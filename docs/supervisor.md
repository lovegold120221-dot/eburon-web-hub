# Running Eburon Hub under a process supervisor
...
Or set ``EBURON_WEBUI_FOREGROUND=1`` in the environment. The Web UI will
...
``~/Library/LaunchAgents/com.example.eburon-webui.plist``:
...
<dict>
    <key>Label</key>
    <string>com.example.eburon-webui</string>

    <key>ProgramArguments</key>
    <array>
        <string>/bin/bash</string>
        <string>/Users/yourname/eburon-webui/start.sh</string>
        <string>--foreground</string>
    </array>

    <key>WorkingDirectory</key>
    <string>/Users/yourname/eburon-webui</string>
    ...
    <string>/Users/yourname/.eburon/webui/launchd-stdout.log</string>

    <key>StandardErrorPath</key>
    <string>/Users/yourname/.eburon/webui/launchd-stderr.log</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.example.eburon-webui.plist
launchctl print gui/$(id -u)/com.example.eburon-webui   # check state
```

Reload after editing the plist:

```bash
launchctl unload ~/Library/LaunchAgents/com.example.eburon-webui.plist
launchctl load   ~/Library/LaunchAgents/com.example.eburon-webui.plist
```
...
``~/.config/systemd/user/eburon-webui.service``:

```ini
[Unit]
Description=Eburon Hub
After=network.target

[Service]
Type=simple
WorkingDirectory=%h/eburon-webui
ExecStart=/bin/bash %h/eburon-webui/start.sh --foreground
Restart=on-failure
RestartSec=5
...
systemctl --user enable --now eburon-webui.service
journalctl --user -u eburon-webui.service -f
```
...
``/etc/supervisor/conf.d/eburon-webui.conf``:

```ini
[program:eburon-webui]
command=/bin/bash /home/youruser/eburon-webui/start.sh --foreground
directory=/home/youruser/eburon-webui
user=youruser
autostart=true
autorestart=true
stopsignal=TERM
stopwaitsecs=10
stdout_logfile=/var/log/eburon-webui.out.log
stderr_logfile=/var/log/eburon-webui.err.log
...
sudo supervisorctl status eburon-webui
```
...
| ``EBURON_WEBUI_FOREGROUND`` | you | Explicit opt-in; accepts ``1`` / ``true`` / ``yes`` / ``on`` |
...
ignore it — set ``EBURON_WEBUI_FOREGROUND=1`` or pass ``--foreground``
explicitly to be safe.

### Supervisors that are NOT auto-detected

The following set no env var that we can reliably detect. Pass
``--foreground`` (or ``EBURON_WEBUI_FOREGROUND=1``) explicitly:
...
respawn loop, set ``EBURON_WEBUI_FOREGROUND=1`` in the service environment.
...
Eburon Hub exposes two health levels:
...
- ``/health?deep=1`` — readiness probe that briefly acquires the stream lock,
  reads the sidebar/session path, reads projects state, and touches Eburon Hub
...
#!/usr/bin/env bash
set -euo pipefail
LABEL="com.example.eburon-webui"
...
``systemctl --user restart eburon-webui.service`` on failure.
