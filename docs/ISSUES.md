...
### Root Cause
The check lives in **beatrice-agent** (upstream), not eburon-webui:

```
beatrice-agent/tools/browser_tool.py → check_browser_requirements()
```
...
### Fix required
The fix must happen in `beatrice-agent/tools/browser_tool.py`:
