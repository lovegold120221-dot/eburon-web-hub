...
| **All-in-one image** (community fork — third-party, not maintained by us) | Podman 3.4 / multi-arch / supervisord-style preference. | [sunnysktsang/eburon-suite](https://github.com/sunnysktsang/eburon-suite) — see [#1399](https://github.com/nesquena/eburon-webui/issues/1399) for the original discussion |
...
That's it. Your existing `~/.eburon` directory is mounted, your `~/workspace` is browsable, and the WebUI auto-detects your UID/GID from the mounted volume.
...
```
PermissionError: [Errno 13] Permission denied: '/home/eburonwebui/.eburon/...'
```
...
**Symptom**: You set `EBURON_HOME_MODE=0640` (or some other group-readable mode) on your host `.env` file, container starts, then errors out:
```
...
- `EBURON_SKIP_CHMOD=1` — bypass the fixer entirely
- `EBURON_HOME_MODE=0640` — allow group bits, only strip world-readable

Both are documented in `api/startup.py::fix_credential_permissions()`.

> ⚠️ **Multi-container warning**: `EBURON_HOME_MODE` has DIFFERENT semantics in the agent image vs. the WebUI:
> - **WebUI**: credential FILE mode threshold (`0640` allows group bits on `.env`)
> - **Agent**: `EBURON_HOME` *directory* mode (default `0700`)
>
> `0640` on a directory has no owner-execute bit, so the agent can't traverse its own home → bricked. For multi-container setups, use `EBURON_HOME_MODE=0750` (group-traversable) or `0701` (x-only). The compose files have per-service comments that match each side's semantics.
...
```
!! WARNING: beatrice-agent source not found.
!!   Looked in: /home/eburonwebui/.eburon/beatrice-agent
!!              /opt/eburon
```

**Cause**: The agent's source (`/opt/eburon` inside the agent container) needs to be exposed to the WebUI container via a shared volume. The two-container compose file does this via `beatrice-agent-src` named volume, but if you're using bind mounts incorrectly the path won't resolve.

**Fix**: Use the named volumes that ship with `docker-compose.two-container.yml` — don't replace them with bind mounts unless you know what you're doing. The agent container writes its source to `/opt/eburon`, and the WebUI mounts that volume at `/home/eburonwebui/.eburon/beatrice-agent`.

If you must use a bind mount: pick a host path, then mount it to `/opt/eburon` in the agent container AND `/home/eburonwebui/.eburon/beatrice-agent` in the WebUI container.
...
- **Combined image** ([sunnysktsang/eburon-suite](https://github.com/sunnysktsang/eburon-suite)) — community fork that ships agent+webui+dashboard in one container

### 6. "config.yaml not loaded"

**Symptom**: You have a `config.yaml` in your host `~/.eburon/`, but the WebUI shows "no model configured" or doesn't pick up your custom providers.

**Cause**: Either the file isn't readable (UID/GID issue, see #1) or it's not in the expected path inside the container.

**Fix**:
- Verify: `docker exec eburon-webui ls -la /home/eburonwebui/.eburon/config.yaml`
...
### 7. "On Podman: can't share .eburon between containers"
...
**Fix**: Either upgrade to Podman 4+ (which fixes this), or use the [single-container setup](#5-minute-quickstart-single-container), or use the [community all-in-one image](https://github.com/sunnysktsang/eburon-suite).
...
```
      ...
      └──────────────┘                        └──────────────┘
              ...
              ↓                                       │
      ┌─────────────────────────┐                     │
      │ beatrice-agent-src (vol)  │─────────────────────┘
      │ (agent's Python source) │
      └─────────────────────────┘
```

The WebUI container doesn't ship with the agent's Python deps — at startup it runs `uv pip install /home/eburonwebui/.eburon/beatrice-agent` to install them from the shared volume.

## Bind-mount migration (advanced)

If you really need to bind-mount an existing host `~/.eburon` (e.g. you're keeping config in dotfiles, sharing with a non-Docker `eburon` install, etc.):
...
volumes:
  eburon-home:
    ...
    driver_opts:
      type: none
      o: bind
      device: /home/youruser/.eburon
  beatrice-agent-src:
    driver: local
    driver_opts:
      type: none
      o: bind
      device: /opt/beatrice-agent-source
```

**Critical requirements**:

1. The host directory MUST be readable by your container UID. Run `id -u` on the host and ensure `~/.eburon` is owned by that UID (or readable via group bits).
2. ALL containers sharing the volume must run as the SAME UID/GID. Set `UID=$(id -u)` and `GID=$(id -g)` in `.env`.
3. If your host `.env` is mode 0640, set `EBURON_SKIP_CHMOD=1` or `EBURON_HOME_MODE=0640` so the startup hook doesn't try to enforce 0600.
...
- #1389 — `EBURON_HOME_MODE` override (fixed in v0.50.254 — agent honors `EBURON_SKIP_CHMOD` and `EBURON_HOME_MODE`)
- #1399 — UID alignment in compose files (fixed in v0.50.260 via PR #1428 + this guide)
- #858 — two-container `/opt/eburon` path confusion
...
If you hit a new failure mode not covered here, please [open an issue](https://github.com/nesquena/eburon-webui/issues/new) with:

1. Which compose file you used
2. The error from `docker logs eburon-webui`
3. `docker exec eburon-webui id` output
4. `docker exec eburon-webui ls -la /home/eburonwebui/.eburon` output
