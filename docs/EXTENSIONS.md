...
Eburon Hub supports a small, opt-in extension surface for self-hosted installs.
...
> Do not point `EBURON_WEBUI_EXTENSION_DIR` at a user-writable directory.
...
panels that should not live in core Eburon Hub.
...
- change Beatrice Agent permissions, models, memory, or tools unless they call
...
before starting the WebUI server. `EBURON_WEBUI_EXTENSION_DIR` must point to an
existing directory before any script or stylesheet URLs are injected:

```bash
export EBURON_WEBUI_EXTENSION_DIR=/path/to/my-extension/static
export EBURON_WEBUI_EXTENSION_SCRIPT_URLS=/extensions/app.js
export EBURON_WEBUI_EXTENSION_STYLESHEET_URLS=/extensions/app.css
./start.sh
```
...
```bash
export EBURON_WEBUI_EXTENSION_SCRIPT_URLS=/extensions/runtime.js,/extensions/app.js
export EBURON_WEBUI_EXTENSION_STYLESHEET_URLS=/extensions/base.css,/extensions/theme.css
```
...
When `EBURON_WEBUI_EXTENSION_DIR` points at an existing directory, files under
...
- keep `EBURON_WEBUI_PASSWORD` enabled
...
```bash
mkdir -p ~/.eburon/webui-extension
cat > ~/.eburon/webui-extension/app.css <<'CSS'
.my-extension-badge {
  position: fixed;
  right: 12px;
  bottom: 12px;
...
CSS
cat > ~/.eburon/webui-extension/app.js <<'JS'
(() => {
...
```bash
EBURON_WEBUI_EXTENSION_DIR=~/.eburon/webui-extension \
EBURON_WEBUI_EXTENSION_STYLESHEET_URLS=/extensions/app.css \
EBURON_WEBUI_EXTENSION_SCRIPT_URLS=/extensions/app.js \
./start.sh
```
