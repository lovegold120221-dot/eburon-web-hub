...
Eburon Hub runs well under WSL2, but native Windows login does not automatically start Linux user processes. This guide covers two supported options:
...
```text
scripts/wsl/eburon_webui_autostart.sh
```
...
|---|---|---|
| `EBURON_WEBUI_REPO` | repo containing the script | WebUI checkout to start |
| `EBURON_WEBUI_LOG_DIR` | `$HOME/.eburon/webui/logs` | Autostart and WebUI logs |
| `EBURON_WEBUI_HOST` | `127.0.0.1` | Host passed through to `start.sh` / `bootstrap.py` |
| `EBURON_WEBUI_PORT` | `8787` | WebUI port and health-check port |
| `EBURON_WEBUI_HEALTH_URL` | `http://127.0.0.1:$EBURON_WEBUI_PORT/health` | URL used to decide whether WebUI is already running |
| `EBURON_WEBUI_PID_FILE` | `$EBURON_WEBUI_LOG_DIR/eburon-webui.pid` | pid file used for duplicate prevention |
| `EBURON_WEBUI_REQUIRE_AGENT_PROCESS` | `0` | Optional: set to `1` only if your local setup requires a separate Eburon Hub process before WebUI starts |

Make the script executable once inside WSL:

```bash
cd /path/to/eburon-webui
chmod +x scripts/wsl/eburon_webui_autostart.sh
```
...
```text
$HOME/.eburon/webui/logs/webui_autostart.log
$HOME/.eburon/webui/logs/eburon_webui.log
```
...
```bash
if [ -x "$HOME/eburon-webui/scripts/wsl/eburon_webui_autostart.sh" ]; then
  EBURON_WEBUI_REPO="$HOME/eburon-webui" \
    "$HOME/eburon-webui/scripts/wsl/eburon_webui_autostart.sh" >/dev/null 2>&1 &
fi
```
...
.\scripts\windows\setup_webui_autostart.ps1 `
  -WslScriptPath "/home/your-user/eburon-webui/scripts/wsl/eburon_webui_autostart.sh" `
  -Distro "Ubuntu"
```
...
- The default task name is `Eburon HubWebUIAutoStart`; pass `-TaskName` if you need a different name.
...
```powershell
Get-ScheduledTask -TaskName Eburon HubWebUIAutoStart
Unregister-ScheduledTask -TaskName Eburon HubWebUIAutoStart -Confirm:$false
```
...
```bash
tail -n 80 "$HOME/.eburon/webui/logs/webui_autostart.log"
tail -n 80 "$HOME/.eburon/webui/logs/eburon_webui.log"
```
...
| Health check fails but pid exists | WebUI is still booting or the port differs | Check `EBURON_WEBUI_PORT` and `eburon_webui.log` |
