"""
Eburon Hub -- Main server entry point.
...
# Load .env before any config imports so env vars are available everywhere
try:
...
class Handler(BaseHTTPRequestHandler):
    ...
    server_version = ('EburonHub/' + _ver_suffix) if _ver_suffix != 'unknown' else 'EburonHub'
...
def main() -> None:
    from api.config import print_startup_config, verify_eburon_imports, _EBURON_FOUND
    ...
    if HOST not in ('127.0.0.1', '::1', 'localhost') and not is_auth_enabled():
        ...
        print(f'     Set a password via Settings or EBURON_HUB_PASSWORD env var.', flush=True)
        ...
        print(f'        and memory via the local API. Set EBURON_HUB_PASSWORD to', flush=True)
        print(f'        enable authentication.', flush=True)

    ok, missing, errors = verify_eburon_imports()
    if not ok and _EBURON_FOUND:
        print(f'[!!] Warning: Beatrice agent found but missing modules: {missing}', flush=True)
    ...
    print(f'  Eburon Hub listening on {scheme}://{HOST}:{PORT}', flush=True)
