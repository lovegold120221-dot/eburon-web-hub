#!/usr/bin/env python3
"""One-shot bootstrap launcher for Eburon Hub."""
...
def _load_repo_dotenv() -> None:
    ...
    Only loads the webui repo .env — not ~/.eburon/.env, which the server
...
DEFAULT_HOST = os.getenv("EBURON_HUB_HOST", "127.0.0.1")
DEFAULT_PORT = int(os.getenv("EBURON_HUB_PORT", "8787"))
# Set EBURON_HUB_SKIP_ONBOARDING=1 to bypass the first-run wizard when
...
def _agent_dir_from_eburon_cli() -> Path | None:
    ...
    (`EBURON_HUB_AGENT_DIR`, `$EBURON_HOME/hermes-agent`, etc.) has missed.
...
def discover_agent_dir() -> Path | None:
    home = Path(os.getenv("EBURON_HOME", str(Path.home() / ".hermes"))).expanduser()
    candidates = [
        os.getenv("EBURON_HUB_AGENT_DIR", ""),
    ...
    for raw in candidates:
        ...
        if candidate.exists() and (candidate / "run_agent.py").exists():
            return candidate
    return _agent_dir_from_eburon_cli()


def discover_launcher_python(agent_dir: Path | None) -> str:
    env_python = os.getenv("EBURON_HUB_PYTHON")
    if env_python:
        return env_python
    if agent_dir:
...
def ensure_python_has_webui_deps(python_exe: str, agent_dir: Path | None = None) -> str:
    ...
    raise RuntimeError(
        ...
        "Set EBURON_HUB_PYTHON to the Hermes Agent venv Python or install the "
        "WebUI requirements into that environment."
    )


def eburon_command_exists() -> bool:
    return shutil.which("hermes") is not None


def install_eburon_agent() -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bootstrap Eburon Hub onboarding.")
...
# - EBURON_HUB_FOREGROUND  explicit user opt-in (=1 / true / yes / on)
#
...
def _detect_supervisor() -> str | None:
    ...
    """
    explicit = os.environ.get("EBURON_HUB_FOREGROUND", "").strip().lower()
    if explicit in ("1", "true", "yes", "on"):
        return "EBURON_HUB_FOREGROUND"
...
def main() -> int:
    ...
    if not agent_dir and not eburon_command_exists():
        if args.skip_agent_install:
            raise RuntimeError(
                "Hermes Agent was not found and auto-install was disabled."
            )
        install_eburon_agent()
        agent_dir = discover_agent_dir()

    python_exe = ensure_python_has_webui_deps(discover_launcher_python(agent_dir), agent_dir)
    state_dir = Path(
        os.getenv("EBURON_HUB_STATE_DIR", str(Path.home() / ".hermes" / "webui"))
    ).expanduser()
    state_dir.mkdir(parents=True, exist_ok=True)

    # Mutate os.environ so child (or post-execv) inherits the resolved values.
    os.environ["EBURON_HUB_HOST"] = args.host
    os.environ["EBURON_HUB_PORT"] = str(args.port)
    os.environ.setdefault("EBURON_HUB_STATE_DIR", str(state_dir))
    if agent_dir:
        os.environ["EBURON_HUB_AGENT_DIR"] = str(agent_dir)
    ...
    if foreground_reason:
        info(
            f"Starting Eburon Hub on http://{args.host}:{args.port} "
        ...
        if not os.access(python_exe, os.X_OK):
            raise RuntimeError(
                ...
                f"Set EBURON_HUB_PYTHON to a working interpreter or fix "
    ...
    info(f"Starting Eburon Hub on http://{args.host}:{args.port}")
