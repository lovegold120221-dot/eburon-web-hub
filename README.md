...
**vs. the field** *(landscape is actively shifting — see [EBURON.md](EBURON.md) for the full breakdown)*:
...
ecosystem. See [EBURON.md](EBURON.md) for the full side-by-side.
...
```bash
git clone https://github.com/lovegold120221-dot/eburon-web-hub.git eburon-web-hub
cd eburon-web-hub
...
```bash
git clone https://github.com/lovegold120221-dot/eburon-web-hub.git
cd eburon-web-hub
...
```bash
docker pull ghcr.io/lovegold120221-dot/eburon-web-hub:latest
docker run -d \
  ...
  ghcr.io/lovegold120221-dot/eburon-web-hub:latest
```

### Build locally

```bash
docker build -t eburon-web-hub .
docker run -d \
  -e WANTED_UID=$(id -u) -e WANTED_GID=$(id -g) \
  -v ~/.eburon:/home/eburonwebui/.eburon \
  -e EBURON_WEBUI_STATE_DIR=/home/eburonwebui/.eburon/webui \
...
- `EBURON.md` -- why Eburon Hub, mental model, and detailed comparison to Claude Code / Codex / OpenCode / Cursor
...
```
git@github.com:lovegold120221-dot/eburon-web-hub.git
```
