# Eburon Hub: Browser Testing Plan
...
SETUP: Clear localStorage (DevTools > Application > Local Storage > delete eburon-webui-session) or open in incognito.
STEPS:
...
EXPECT:
  ...
  - Sidebar footer shows a single "Eburon Hub" control-center button
...
EXPECT:
  ...
  - After a few seconds, Eburon Hub responds
...
EXPECT:
  ...
  - Within 10-30 seconds, Eburon Hub responds with a three-word greeting
...
EXPECT:
  ...
  - Eburon Hub responds describing or reading the file content
FAIL: Upload fails, file badge never appears, Eburon Hub does not mention the file.
...
STEPS:
  1. Type: "Run the command: rm -rf /tmp/eburon_test_delete_me"
  2. Send the message
EXPECT:
...
SETUP: Create a safe test: type "Run: touch /tmp/eburon_approval_test.txt"
STEPS:
  1. Send the message
...
EXPECT:
  ...
  - Verify: open a terminal and run: ls /tmp/eburon_approval_test.txt
...
STEPS:
  1. Click the "Eburon Hub" button in the sidebar footer
  2. In the Control Center modal, click "Transcript"
EXPECT:
  - Browser downloads a .md file named eburon-{session_id}.md
...
4. For T7.x (approval tests): the agent running inside Eburon Hub needs to detect a
   dangerous command. Ask Eburon Hub to "run: rm -rf /tmp/test_eburon_approval" and watch
...
EXPECT:
  - A small clipboard icon appears in the message header row (right side of "You" or "Eburon Hub")
...
EXPECT:
  - "You" and "Eburon Hub" labels appear in slightly muted blue/gold (not full-brightness)
...
EXPECT:
  - Browser downloads a file named eburon-{session_id}.json
...
EXPECT:
  ...
  - Eburon Hub streams a fresh response
...
EXPECT:
  ...
  - Eburon Hub streams a new response
...
EXPECT:
  - The "Eburon Hub" button is visible in the sidebar footer
...
SETUP: Ask Eburon Hub something that produces a code response (e.g. "Show me a Python hello world").
EXPECT:
...
STEPS:
  1. While Eburon Hub is responding, type a new message and press Enter
EXPECT:
...
- Click the "Eburon Hub" button in the sidebar footer. Control Center overlay opens with vertical section tabs on the left.
...
- Set `EBURON_WEBUI_PASSWORD=test` env var. Restart. All pages redirect to `/login`.
...
- With beatrice-agent not installed → only default profile shown, graceful fallback.

---

## Slash command parity (manual checklist)

For each batch-1 command, run via webui slash menu AND via `eburon` CLI in the
same `EBURON_HOME` (when applicable) and verify identical effect.
