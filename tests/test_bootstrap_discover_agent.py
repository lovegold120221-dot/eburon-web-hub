...
`<webui-parent>/hermes-agent`, `EBURON_HUB_AGENT_DIR`) don't match, bootstrap
...
def _isolate_discover_agent_dir(monkeypatch, tmp_path, hermes_path):
    ...
    monkeypatch.setenv("EBURON_HOME", str(tmp_path / "no-such-hermes-home"))
...
def test_explicit_candidate_takes_precedence_over_shebang(monkeypatch, tmp_path):
    """EBURON_HUB_AGENT_DIR and the standard layout still win when present."""
    ...
    hermes = _make_hermes_cli(tmp_path, str(venv_python))
    _isolate_discover_agent_dir(monkeypatch, tmp_path, hermes)
    monkeypatch.setenv("EBURON_HUB_AGENT_DIR", str(explicit_install))
