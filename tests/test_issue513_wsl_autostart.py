...
def test_wsl_autostart_docs_cover_session_and_task_scheduler_options():
    ...
    assert "EBURON_HUB_REPO" in doc
    assert "EBURON_HUB_LOG_DIR" in doc
    assert "EBURON_HUB_REQUIRE_AGENT_PROCESS" in doc
...
def test_wsl_autostart_launcher_has_safe_duplicate_prevention_and_exports_runtime_env():
    ...
    assert "EBURON_HUB_LOCK_FILE" in script
    assert "EBURON_HUB_PID_FILE" in script
    assert "curl -fsS --max-time 3" in script
    assert "bash \"${EBURON_HUB_REPO}/start.sh\" --foreground" in script
    assert "nohup" in script

    # The launcher documents EBURON_HUB_HOST/PORT as runtime knobs; they must
    # be exported so bootstrap.py/server.py receive the selected WSL values.
    assert re.search(r"^export EBURON_HUB_HOST EBURON_HUB_PORT$", script, re.MULTILINE)
