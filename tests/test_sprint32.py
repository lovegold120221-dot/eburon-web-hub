...
class TestAutoInstallAgentDeps:
    ...
    All tests that exercise the install path set EBURON_HUB_AUTO_INSTALL=1
    ...
    def test_disabled_by_default(self, tmp_path, capsys):
        """Auto-install must be off unless EBURON_HUB_AUTO_INSTALL=1 is set."""
        agent_dir = tmp_path / 'hermes-agent'
        agent_dir.mkdir()
        (agent_dir / 'requirements.txt').write_text('somepkg\n')
        env = {'EBURON_HUB_AGENT_DIR': str(agent_dir)}
        with patch.dict('os.environ', env, clear=False):
            os.environ.pop('EBURON_HUB_AUTO_INSTALL', None)
    ...
    def test_installs_from_requirements_txt(self, tmp_path):
        ...
        env = {'EBURON_HUB_AGENT_DIR': str(agent_dir), 'EBURON_HUB_AUTO_INSTALL': '1'}
        with patch.dict('os.environ', env, clear=False):
    ...
    def test_skips_when_agent_dir_missing(self, tmp_path, capsys):
        ...
        env_overrides = {
            'EBURON_HUB_AGENT_DIR': str(missing),
            'EBURON_HOME': str(tmp_path / 'no-hermes-home'),
            'EBURON_HUB_AUTO_INSTALL': '1',
        }
