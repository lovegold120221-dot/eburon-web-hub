...
EBURON_HUB_PASSWORD env var is set.
...
def test_get_settings_exposes_password_env_var_true_when_env_set(monkeypatch):
    ...
    when EBURON_HUB_PASSWORD is set."""
    monkeypatch.setenv("EBURON_HUB_PASSWORD", "shadow-pw")
    ...
    assert payload.get("password_env_var") is True, (
        ...
        "EBURON_HUB_PASSWORD is set so the UI can disable the password field. "
        f"Got: {payload!r}"
    )
...
def test_get_settings_password_env_var_false_when_env_unset(monkeypatch):
    ...
    monkeypatch.delenv("EBURON_HUB_PASSWORD", raising=False)
...
def test_get_settings_password_env_var_false_when_env_blank(monkeypatch):
    ...
    monkeypatch.setenv("EBURON_HUB_PASSWORD", "   ")
...
def test_post_set_password_returns_409_when_env_var_set(monkeypatch):
    ...
    with a message naming EBURON_HUB_PASSWORD so the user knows what to fix."""
    monkeypatch.setenv("EBURON_HUB_PASSWORD", "shadow-pw")
    ...
    payload = handler.json_body()
    assert "EBURON_HUB_PASSWORD" in payload.get("error", ""), (
        "409 error message must name EBURON_HUB_PASSWORD so the user can "
