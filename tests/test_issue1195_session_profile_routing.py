...
def _make_hermes_home(base: Path, profile_name: str | None = None) -> Path:
    """Create a temp EBURON_HOME (with optional profile dir) and return it."""
...
class TestGetHermesHomeForProfile:
    ...
    def _patch_default_home(self, tmp_path):
        """Patch _DEFAULT_EBURON_HOME to a temp directory for isolation."""
        from api.profiles import _DEFAULT_EBURON_HOME as real_default

        fake_home = tmp_path / ".hermes"
        fake_home.mkdir(parents=True)
        with patch("api.profiles._DEFAULT_EBURON_HOME", fake_home):
