...
get_hermes_home_for_profile() resolves a EBURON_HOME path from a name without
...
def test_get_hermes_home_for_profile_returns_profile_subdir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(p, '_DEFAULT_EBURON_HOME', tmp_path)
...
def test_get_hermes_home_for_profile_does_not_mutate_globals():
    ...
    before_hermes_home = os.environ.get('EBURON_HOME')
    ...
    assert os.environ.get('EBURON_HOME') == before_hermes_home, (
        "get_hermes_home_for_profile() must not mutate os.environ['EBURON_HOME']"
    )
...
print(json.dumps({
    'default_home': str(p._DEFAULT_EBURON_HOME),
...
def test_hermes_base_home_named_profile_matches_cookie_without_doubling(tmp_path):
    ...
    EBURON_HOME set to /base/profiles/foo while the browser still sends the
...
# to SESSION_DIR which is set from EBURON_HUB_STATE_DIR at import time and may
