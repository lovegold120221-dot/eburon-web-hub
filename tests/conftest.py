...
PATH DISCOVERY:
  No hardcoded paths. Discovery order:
    1. Environment variables (EBURON_HUB_AGENT_DIR, EBURON_HUB_PYTHON, etc.)
...
EBURON_HOME = pathlib.Path(os.getenv('EBURON_HOME', str(HOME / '.hermes')))
...
# Override with EBURON_HUB_TEST_PORT / EBURON_HUB_TEST_STATE_DIR to pin.
...
def _auto_test_port(repo_root) -> int:
    ...
    Override with EBURON_HUB_TEST_PORT to use a specific port."""
    import hashlib
...
TEST_PORT      = int(os.getenv('EBURON_HUB_TEST_PORT',
                               str(_auto_test_port(REPO_ROOT))))
TEST_BASE      = f"http://127.0.0.1:{TEST_PORT}"
TEST_STATE_DIR = pathlib.Path(os.getenv(
    'EBURON_HUB_TEST_STATE_DIR',
    str(EBURON_HOME / _auto_state_dir_name(REPO_ROOT))
))
...
os.environ['EBURON_HUB_TEST_PORT'] = str(TEST_PORT)
os.environ['EBURON_HUB_TEST_STATE_DIR'] = str(TEST_STATE_DIR)
os.environ['EBURON_HUB_STATE_DIR'] = str(TEST_STATE_DIR)
os.environ['EBURON_HUB_DEFAULT_WORKSPACE'] = str(TEST_WORKSPACE)
os.environ['EBURON_HOME'] = str(TEST_STATE_DIR)
...
def test_server():
    ...
    real_skills  = EBURON_HOME / 'skills'
    ...
    # EBURON_HUB_STATE_DIR for their own purposes, but EBURON_HUB_TEST_STATE_DIR
    ...
    os.environ.setdefault('EBURON_HUB_TEST_PORT', str(TEST_PORT))
    ...
    for _k in list(env):
        if any(_k.startswith(p) for p in (
            ...
            del env[_k]
    env.update({
        "EBURON_HUB_PORT":              str(TEST_PORT),
        "EBURON_HUB_HOST":              "127.0.0.1",
        "EBURON_HUB_STATE_DIR":         str(TEST_STATE_DIR),
        "EBURON_HUB_DEFAULT_WORKSPACE": str(TEST_WORKSPACE),
        "EBURON_HUB_DEFAULT_MODEL":     "openai/gpt-5.4-mini",
        "EBURON_HOME":                    str(TEST_STATE_DIR),
        "HERMES_CONFIG_PATH":             str(TEST_STATE_DIR / 'config.yaml'),
        # Belt-and-suspenders: HERMES_BASE_HOME hard-locks _DEFAULT_EBURON_HOME
    ...
    if HERMES_AGENT:
        env["EBURON_HUB_AGENT_DIR"] = str(HERMES_AGENT)
