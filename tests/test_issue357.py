...
class TestWorkspacePermissions:
    ...
    def test_workspace_uses_sudo_mkdir(self):
        ...
        ws_section = INIT_SCRIPT[
            INIT_SCRIPT.find("EBURON_HUB_DEFAULT_WORKSPACE"):
            INIT_SCRIPT.find("EBURON_HUB_DEFAULT_WORKSPACE") + 800
        ]
    ...
    def test_workspace_uses_sudo_chown(self):
        ...
        """
        assert 'sudo chown hermeswebui:hermeswebui "$EBURON_HUB_DEFAULT_WORKSPACE"' in INIT_SCRIPT, (
    ...
    def test_workspace_mkdir_before_chown(self):
        ...
        mkdir_pos = INIT_SCRIPT.find('sudo mkdir -p "$EBURON_HUB_DEFAULT_WORKSPACE"')
        chown_pos = INIT_SCRIPT.find('sudo chown hermeswebui:hermeswebui "$EBURON_HUB_DEFAULT_WORKSPACE"')
    ...
    def test_workspace_error_exit_on_mkdir_failure(self):
        ...
        assert 'sudo mkdir -p "$EBURON_HUB_DEFAULT_WORKSPACE" || error_exit' in INIT_SCRIPT, (
    ...
    def test_workspace_chown_is_conditional_on_writable(self):
        ...
        The script must check [ -w "$EBURON_HUB_DEFAULT_WORKSPACE" ] before
        attempting chown or a write test, so :ro bind-mounts don't crash startup.
        """
        assert '[ -w "$EBURON_HUB_DEFAULT_WORKSPACE" ]' in INIT_SCRIPT, (
