"""Tests for issue #1560 — Settings password silently no-ops when EBURON_HUB_PASSWORD env var is set.

Root cause: EBURON_HUB_PASSWORD takes precedence in api.auth.get_password_hash(),
...
def test_get_settings_surfaces_password_env_var_flag():
    ...
    assert 'EBURON_HUB_PASSWORD' in block, \
        'GET /api/settings must read EBURON_HUB_PASSWORD env var'
...
def test_post_settings_refuses_set_password_when_env_var_shadowed():
    ...
    assert 'EBURON_HUB_PASSWORD env var is set' in src, \
...
def test_post_settings_refuses_clear_password_when_env_var_shadowed():
    ...
    guard_idx = src.index('EBURON_HUB_PASSWORD env var is set')
...
def test_i18n_locked_string_mentions_env_var_name_in_all_locales():
    """Each locale's banner must literally mention EBURON_HUB_PASSWORD so users can find it."""
    src = _read('static/i18n.js')
    blocks = _split_locales(src)
    for loc in LOCALES:
        ...
        assert 'EBURON_HUB_PASSWORD' in line, \
            f"{loc}: banner must literally name EBURON_HUB_PASSWORD"
...
def test_get_settings_returns_password_env_var_false_when_unset():
    """When EBURON_HUB_PASSWORD is not set in the test process,
    GET /api/settings must include `password_env_var: False`."""
    # The conftest server inherits this process's env; verify it's clean.
    assert not os.getenv('EBURON_HUB_PASSWORD', '').strip(), \
        'this test requires EBURON_HUB_PASSWORD to be unset'
