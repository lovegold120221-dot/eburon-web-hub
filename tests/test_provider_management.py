...
class TestSetProviderKey:
    ...
    def test_set_key_writes_to_env_file(self, monkeypatch, tmp_path):
        ...
        _install_fake_hermes_cli(monkeypatch)
        monkeypatch.setattr(profiles, "get_active_hermes_home", lambda: tmp_path)
        # Also pin EBURON_HOME so code that reads it directly gets tmp_path,
        # not the conftest session TEST_STATE_DIR that bleeds into the main process.
        monkeypatch.setenv("EBURON_HOME", str(tmp_path))
        ...
        try:
            ...
            assert env_path.exists(), f".env not written to {env_path}; EBURON_HOME={__import__('os').environ.get('EBURON_HOME')!r}"
