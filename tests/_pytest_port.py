...
conftest.py publishes ``EBURON_HUB_TEST_PORT`` and
``EBURON_HUB_TEST_STATE_DIR`` to ``os.environ`` at module level
...
_EBURON_HOME = pathlib.Path(os.getenv('EBURON_HOME',
                             str(pathlib.Path.home() / '.hermes')))

TEST_PORT = int(os.environ.get('EBURON_HUB_TEST_PORT',
                               str(_auto_test_port(_REPO_ROOT))))
BASE = f"http://127.0.0.1:{TEST_PORT}"

TEST_STATE_DIR = pathlib.Path(os.environ.get(
    'EBURON_HUB_TEST_STATE_DIR',
    str(_EBURON_HOME / _auto_state_dir_name(_REPO_ROOT))
))
...
TEST_DEFAULT_MODEL = os.environ.get('EBURON_HUB_DEFAULT_MODEL', 'openai/gpt-5.4-mini')
