...
def test_extension_config_disabled_by_default(monkeypatch):
    monkeypatch.delenv("EBURON_HUB_EXTENSION_DIR", raising=False)
    monkeypatch.delenv("EBURON_HUB_EXTENSION_SCRIPT_URLS", raising=False)
    monkeypatch.delenv("EBURON_HUB_EXTENSION_STYLESHEET_URLS", raising=False)
...
def test_extension_config_accepts_only_safe_same_origin_urls(tmp_path, monkeypatch):
    ...
    root.mkdir()
    monkeypatch.setenv("EBURON_HUB_EXTENSION_DIR", str(root))
    monkeypatch.setenv(
        "EBURON_HUB_EXTENSION_SCRIPT_URLS",
        ", ".join(
            [
                "/extensions/app.js",
    ...
    monkeypatch.setenv(
        "EBURON_HUB_EXTENSION_STYLESHEET_URLS",
...
def test_index_html_injection_escapes_urls_and_preserves_disabled_default(tmp_path, monkeypatch):
    monkeypatch.delenv("EBURON_HUB_EXTENSION_DIR", raising=False)
    monkeypatch.setenv("EBURON_HUB_EXTENSION_SCRIPT_URLS", "/extensions/app.js")
    ...
    root.mkdir()
    monkeypatch.setenv("EBURON_HUB_EXTENSION_DIR", str(root))
    monkeypatch.setenv("EBURON_HUB_EXTENSION_SCRIPT_URLS", "/extensions/app.js?v=1&mode=dev")
    monkeypatch.setenv("EBURON_HUB_EXTENSION_STYLESHEET_URLS", "/extensions/app.css")
...
def test_extension_route_remains_behind_webui_auth(monkeypatch):
    monkeypatch.setenv("EBURON_HUB_PASSWORD", "test-password")
...
def test_extension_static_serving_fails_closed_when_disabled_or_unreadable(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("EBURON_HUB_EXTENSION_DIR", str(missing_root))
