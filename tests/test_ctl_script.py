...
def run_ctl(
    ...
    *args: str,
    ...
    for key in (
        ...
        "EBURON_HUB_PORT",
        "EBURON_HUB_PYTHON",
        "EBURON_HUB_STATE_DIR",
        "EBURON_HUB_PID_FILE",
        "EBURON_HUB_LOG_FILE",
        "EBURON_HUB_CTL_STATE_FILE",
    ):
        merged.pop(key, None)
    merged.update(
        {
            "HOME": str(home),
            "EBURON_HOME": str(home / ".hermes"),
...
def write_fake_python(path: Path) -> None:
    path.write_text(
        textwrap.dedent(
            ...
            printf 'host=%s port=%s state=%s\n' "${EBURON_HUB_HOST:-}" "${EBURON_HUB_PORT:-}" "${EBURON_HUB_STATE_DIR:-}" >> "${FAKE_PYTHON_LOG}"
...
def test_start_writes_pid_under_hermes_home_runs_foreground_no_browser_and_logs(tmp_path):
    ...
    result = run_ctl(
        ...
        "start",
        env={
            "EBURON_HUB_PYTHON": str(fake_python),
            "FAKE_PYTHON_LOG": str(fake_log),
            "EBURON_HUB_HOST": "0.0.0.0",
            "EBURON_HUB_PORT": "18991",
        },
    )
...
def test_start_loads_dotenv_but_inline_overrides_win(tmp_path):
    ...
    (repo_root / ".env").write_text(
        "EBURON_HUB_HOST=127.9.9.9\nEBURON_HUB_PORT=18888\n",
        encoding="utf-8",
    )
