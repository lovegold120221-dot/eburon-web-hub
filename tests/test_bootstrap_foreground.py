...
4.  ``_detect_supervisor()`` returns ``EBURON_HUB_FOREGROUND`` for the
    explicit opt-in, accepting ``1``/``true``/``yes``/``on`` (case-insensitive)
5.  ``_detect_supervisor()`` ignores ``EBURON_HUB_FOREGROUND=0`` /
...
11. Foreground path exports ``EBURON_HUB_HOST`` / ``EBURON_HUB_PORT`` /
    ``EBURON_HUB_AGENT_DIR`` / ``EBURON_HUB_STATE_DIR`` to ``os.environ``
...
def clean_env(monkeypatch):
    ...
    The resolved-vars stripping (EBURON_HUB_HOST etc.) prevents leakage
...
class TestDetectSupervisor:
    ...
    def test_explicit_opt_in_truthy_values(self, import_bootstrap, clean_env, monkeypatch, value):
        monkeypatch.setenv("EBURON_HUB_FOREGROUND", value)
        assert import_bootstrap._detect_supervisor() == "EBURON_HUB_FOREGROUND"

    @pytest.mark.parametrize("value", ["0", "false", "FALSE", "no", "off", "", "  "])
    def test_explicit_opt_in_falsy_values_fall_through(self, import_bootstrap, clean_env, monkeypatch, value):
        # When EBURON_HUB_FOREGROUND is falsy, we should NOT short-circuit on it.
    ...
    def test_explicit_opt_in_takes_precedence_over_supervisor_var(self, import_bootstrap, clean_env, monkeypatch):
        ...
        monkeypatch.setenv("EBURON_HUB_FOREGROUND", "1")
...
class TestMainForegroundRouting:
    ...
    def stub_main_dependencies(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("EBURON_HUB_STATE_DIR", str(tmp_path / "state"))
...
class TestForegroundEnvAndCwd:
    ...
    def test_foreground_exports_resolved_env_vars(self, setup, monkeypatch, clean_env):
        ...
        assert os.environ["EBURON_HUB_HOST"] == "0.0.0.0"
        assert os.environ["EBURON_HUB_PORT"] == "9119"
        assert os.environ["EBURON_HUB_AGENT_DIR"] == str(agent_dir)
        # state-dir was already set by the fixture; verify it survived.
        assert "EBURON_HUB_STATE_DIR" in os.environ
