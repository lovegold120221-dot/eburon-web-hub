...
def test_codex_account_usage_is_fetched_under_active_profile_home(monkeypatch, tmp_path):
    """Codex account limits must use the selected WebUI profile's EBURON_HOME."""
    monkeypatch.setattr(profiles, "get_active_hermes_home", lambda: tmp_path)
    old_cfg, old_mtime = _with_config(model={"provider": "openai-codex"})

    import api.providers as providers
    seen = {}
    previous_home = os.environ.get("EBURON_HOME")

    def fake_fetch(provider, base_url=None, api_key=None):
        seen["provider"] = provider
        seen["api_key"] = api_key
        seen["hermes_home"] = os.environ.get("EBURON_HOME")
    ...
    assert os.environ.get("EBURON_HOME") == previous_home
