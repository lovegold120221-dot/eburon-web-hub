...
This test suite locks the diagnostic shape of the new error message:
  ...
  - EBURON_HUB_AGENT_DIR is shown if set, "(not set)" otherwise.
...
class TestAIAgentImportErrorDetail:
    ...
    def test_shows_agent_dir_env_when_set(self, monkeypatch):
        """If EBURON_HUB_AGENT_DIR is set, the diagnostic must show its value
        so the user can confirm whether the override is pointing at the right
        directory.
        """
        helper = _import_helper()
        monkeypatch.setenv("EBURON_HUB_AGENT_DIR", "/custom/agent/path")
        out = helper()
        assert "EBURON_HUB_AGENT_DIR: /custom/agent/path" in out

    def test_shows_agent_dir_env_unset_marker(self, monkeypatch):
        """If EBURON_HUB_AGENT_DIR is NOT set, the diagnostic must say so
        explicitly — silence is ambiguous (could be empty string, could be unset).
        """
        helper = _import_helper()
        monkeypatch.delenv("EBURON_HUB_AGENT_DIR", raising=False)
        out = helper()
        assert "EBURON_HUB_AGENT_DIR: (not set)" in out
