"""
Tests for optional TLS/HTTPS support (EBURON_HUB_TLS_CERT / TLS_KEY).
...
def _start_server(port: int, cert: str = None, key: str = None) -> subprocess.Popen:
    ...
    env["EBURON_HUB_HOST"] = "127.0.0.1"
    env["EBURON_HUB_PORT"] = str(port)
    env.pop("EBURON_HUB_TLS_CERT", None)
    env.pop("EBURON_HUB_TLS_KEY", None)
    if cert:
        env["EBURON_HUB_TLS_CERT"] = cert
    if key:
        env["EBURON_HUB_TLS_KEY"] = key
    env["EBURON_HUB_STATE_DIR"] = str(Path(tempfile.mkdtemp()))
...
class TestTLSConfigFlag(unittest.TestCase):
    ...
    def test_tls_enabled_true_when_both_env_set(self):
        code = textwrap.dedent("""\
            import os
            os.environ['EBURON_HUB_TLS_CERT'] = '/tmp/cert.pem'
            os.environ['EBURON_HUB_TLS_KEY'] = '/tmp/key.pem'
    ...
    def test_tls_enabled_false_when_env_absent(self):
        env = {k: v for k, v in os.environ.items()
               if k not in ("EBURON_HUB_TLS_CERT", "EBURON_HUB_TLS_KEY")}
        code = textwrap.dedent("""\
            import os
            os.environ.pop('EBURON_HUB_TLS_CERT', None)
            os.environ.pop('EBURON_HUB_TLS_KEY', None)
            from api.config import TLS_ENABLED
            print(TLS_ENABLED)
        """)
        r = subprocess.run(
    ...
    def test_tls_enabled_false_when_only_cert_set(self):
        env = {k: v for k, v in os.environ.items()
               if k not in ("EBURON_HUB_TLS_CERT", "EBURON_HUB_TLS_KEY")}
        env["EBURON_HUB_TLS_CERT"] = "/tmp/cert.pem"
        code = textwrap.dedent("""\
            from api.config import TLS_ENABLED
            print(TLS_ENABLED)
        """)
