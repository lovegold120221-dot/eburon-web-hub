...
- All compose files document the bind-mount permission escape hatches
  (`HERMES_SKIP_CHMOD`, `EBURON_HOME_MODE`) inline so users hit by #1389
...
def test_compose_files_document_skip_chmod_escape_hatch():
    ...
    for fname in ("docker-compose.yml", "docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        ...
        assert "EBURON_HOME_MODE" in src, (
            f"{fname}: must document EBURON_HOME_MODE alongside HERMES_SKIP_CHMOD"
        )
...
def test_env_docker_example_exists():
    ...
    for var in ("UID", "GID", "EBURON_HOME", "HERMES_WORKSPACE",
                "EBURON_HUB_PASSWORD", "HERMES_SKIP_CHMOD", "EBURON_HOME_MODE"):
        assert var in src, (
...
# ── 7: agent vs webui EBURON_HOME_MODE semantic asymmetry ──────────────────


def test_agent_service_does_not_recommend_invalid_home_mode():
    """REGRESSION (Opus pre-release advisor): the WebUI's EBURON_HOME_MODE
    is a credential-file threshold (0640 = allow group bits). The agent's
    EBURON_HOME_MODE is a DIRECTORY mode (default 0700). 0640 on a directory
    has no owner-execute bit, so the agent can't traverse its own home and
    bricks. The agent service blocks must NOT recommend EBURON_HOME_MODE=0640
    as their example value."""
    import re

    BAD_VALUES = (
        "EBURON_HOME_MODE=0640",
        "EBURON_HOME_MODE=0644",
        "EBURON_HOME_MODE=0600",
        "EBURON_HOME_MODE=0660",
    )
        ...
        for service_name in ("hermes-agent", "hermes-dashboard"):
            ...
            for bad in BAD_VALUES:
                assert bad not in block, (
                    ...
                    f"the agent's EBURON_HOME_MODE applies to the EBURON_HOME "
...
def test_compose_files_warn_about_home_mode_asymmetry():
    ...
    EBURON_HOME_MODE semantic asymmetry so users don't copy the WebUI's
    ...
    for fname in ("docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        ...
        assert "directory" in src and "credential" in src, (
            f"{fname} must contain comments explaining that EBURON_HOME_MODE "
...
def test_env_docker_example_warns_about_home_mode_asymmetry():
    """The .env.docker.example template must warn that EBURON_HOME_MODE has
    ...
    assert "MULTI-CONTAINER WARNING" in src, (
        ...
        "the EBURON_HOME_MODE semantic asymmetry between WebUI and agent."
    )
