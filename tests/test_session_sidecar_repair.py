...
def hermes_home(tmp_path, monkeypatch):
    """Set up a EBURON_HOME directory with a sessions subdirectory."""
    home = tmp_path / "hermes_home"
    home.mkdir()
    sessions_dir = home / "sessions"
    sessions_dir.mkdir()
    monkeypatch.setenv("EBURON_HOME", str(home))
    monkeypatch.setattr(profiles, "_DEFAULT_EBURON_HOME", home)
    return home
...
class TestGetProfileHome:
    ...
    def test_expands_tilde_when_profiles_unavailable(self, monkeypatch):
        """When api.profiles import fails, fallback uses EBURON_HOME or ~/.hermes
        with proper tilde expansion."""
        # Make api.profiles import fail
        monkeypatch.setitem(sys.modules, "api.profiles", None)

        # Default fallback without EBURON_HOME env var
        monkeypatch.delenv("EBURON_HOME", raising=False)
    ...
    def test_uses_hermes_home_env_var(self, monkeypatch):
        """When EBURON_HOME is set, fallback uses it with expansion."""
        monkeypatch.setitem(sys.modules, "api.profiles", None)
        monkeypatch.setenv("EBURON_HOME", "/custom/hermes")
        result = _get_profile_home(None)
        assert str(result) == "/custom/hermes"

    def test_expands_tilde_in_hermes_home(self, monkeypatch):
        """If EBURON_HOME contains ~, it gets expanded."""
        monkeypatch.setitem(sys.modules, "api.profiles", None)
        monkeypatch.setenv("EBURON_HOME", "~/my-hermes")
        result = _get_profile_home(None)
