...
function _beginSettingsPanelSession() {
  ...
  _settingsThemeOnOpen = localStorage.getItem('eburon-theme') || 'dark';
  _settingsSkinOnOpen = localStorage.getItem('eburon-skin') || 'default';
  _settingsFontSizeOnOpen = localStorage.getItem('eburon-font-size') || 'default';
...
const KANBAN_BOARD_LS_KEY = 'eburon-kanban-active-board';
...
function _renderLlmWikiStatus(d) {
  ...
  const rawDocsUrl = status.docs_url || 'https://eburon-hub.example.com/docs/user-guide/skills/bundled/research/research-llm-wiki';
  ...
  const toggleNote = status.toggle_available
    ? 'Toggle available from configured Beatrice Agent setting.'
...
async function switchToProfile(name) {
  ...
  try {
    ...
    else localStorage.removeItem('eburon-webui-model');
    _skillsData = null;
    _workspaceList = null;
...
let _settingsEburonDefaultModelOnOpen = '';
...
function _syncEburonPanelSessionActions(){
...
function _appearancePayloadFromUi(){
  return {
    theme: ($('settingsTheme')||{}).value || localStorage.getItem('eburon-theme') || 'dark',
    skin: ($('settingsSkin')||{}).value || localStorage.getItem('eburon-skin') || 'default',
    font_size: ($('settingsFontSize')||{}).value || localStorage.getItem('eburon-font-size') || 'default',
  };
}
...
function _rememberAppearanceSaved(payload){
  if(!payload) return;
  _settingsThemeOnOpen=payload.theme||localStorage.getItem('eburon-theme')||'dark';
  _settingsSkinOnOpen=payload.skin||localStorage.getItem('eburon-skin')||'default';
  _settingsFontSizeOnOpen=payload.font_size||localStorage.getItem('eburon-font-size')||'default';
}
...
async function _autosaveAppearanceSettings(payload){
  try{
    ...
    if(saved&&saved.font_size){
      localStorage.setItem('eburon-font-size',saved.font_size);
    }
...
function _rememberPreferencesSaved(payload){
  if(!payload) return;
  if(payload.send_key!==undefined) localStorage.setItem('eburon-pref-send_key',payload.send_key);
  if(payload.language!==undefined) localStorage.setItem('eburon-pref-language',payload.language);
}
...
async function _autosavePreferencesSettings(payload){
  try{
    ...
    const modelDirty=!!(modelSel&&((modelSel.value||'')!==(_settingsEburonDefaultModelOnOpen||'')));
...
async function loadSettingsPanel(){
  try{
    ...
    const fontSizeVal=settings.font_size||localStorage.getItem('eburon-font-size')||'default';
    localStorage.setItem('eburon-font-size',fontSizeVal);
    ...
    if(modelSel){
      ...
      _settingsEburonDefaultModelOnOpen=(models&&models.default_model)||'';
      ...
      if(typeof _applyModelToDropdown==='function'){
        _applyModelToDropdown(_settingsEburonDefaultModelOnOpen, modelSel, (models&&models.active_provider)||window._activeProvider||null);
      }else{
        modelSel.value=_settingsEburonDefaultModelOnOpen;
      }
    ...
    if(voiceModeCb){
      voiceModeCb.checked=localStorage.getItem('eburon-voice-mode-button')==='true';
      voiceModeCb.onchange=function(){
        localStorage.setItem('eburon-voice-mode-button',this.checked?'true':'false');
    ...
    if(ttsVoiceSel&&'speechSynthesis' in window){
      const populateVoices=()=>{
        ...
        const current=localStorage.getItem('eburon-tts-voice')||'';
      ...
      ttsVoiceSel.onchange=function(){localStorage.setItem('eburon-tts-voice',this.value);};
    }

    // --- Voice provider (engine) selector + sub-dropdowns -------------------
    const ttsProviderSel=$('settingsTtsProvider');
    const kokoroVoiceSel=$('settingsKokoroVoice');
    const kokoroVoiceRow=$('kokoroVoiceRow');
    const luxProfileSel=$('settingsLuxTtsProfile');
    const luxProfileRow=$('luxTtsProfileRow');

    // Load and populate TTS status for both providers
    async function _loadTtsProviders(){
      try{
        const res=await fetch('/api/tts/status',{credentials:'include'});
        if(!res.ok) return;
        const data=await res.json();
        const providers=data.providers||{};
        const kokoro=providers.kokoro||{};
        const luxtts=providers.luxtts||{};

        // Populate Kokoro voice dropdown
        if(kokoroVoiceSel){
          const voices=Array.isArray(kokoro.voices)?kokoro.voices:[];
          const currentKokoro=localStorage.getItem('hermes-kokoro-voice')||kokoro.default_voice||'af_heart';
          kokoroVoiceSel.innerHTML='';
          // Group by language
          const langs={};
          voices.forEach(v=>{
            const lg=v.lang||'Other';
            if(!langs[lg]) langs[lg]=[];
            langs[lg].push(v);
          });
          Object.keys(langs).sort().forEach(lang=>{
            const group=document.createElement('optgroup');
            group.label=lang;
            langs[lang].forEach(v=>{
              const opt=document.createElement('option');
              opt.value=v.id;
              opt.textContent=v.name;
              if(String(v.id)===String(currentKokoro)) opt.selected=true;
              group.appendChild(opt);
            });
            kokoroVoiceSel.appendChild(group);
          });
        }

        // Populate LuxTTS profile dropdown
        if(luxProfileSel){
          const profiles=Array.isArray(luxtts.profiles)?luxtts.profiles:[];
          const currentLux=localStorage.getItem('hermes-luxtts-profile-id')||'';
          luxProfileSel.innerHTML='';
          profiles.forEach(p=>{
            const opt=document.createElement('option');
            const name=p.name||p.id||'Voice';
            opt.value=p.id||'';
            opt.textContent=name+(p.language?' ('+p.language+')':'')+(p.default_engine?' ['+p.default_engine+']':'');
            if(String(p.id||'')===String(currentLux)) opt.selected=true;
            luxProfileSel.appendChild(opt);
          });
        }
      }catch(e){
        // Silently fail - dropdowns keep their placeholder
      }
    }

    // Show/hide sub-dropdowns based on selected provider
    function _applyProviderUI(){
      const provider=ttsProviderSel?ttsProviderSel.value:'kokoro';
      if(provider==='luxtts'){
        if(kokoroVoiceRow) kokoroVoiceRow.style.display='none';
        if(luxProfileRow) luxProfileRow.style.display='';
      }else{
        if(kokoroVoiceRow) kokoroVoiceRow.style.display='';
        if(luxProfileRow) luxProfileRow.style.display='none';
      }
    }

    if(ttsProviderSel){
      // Restore saved provider
      const savedProvider=localStorage.getItem('hermes-tts-engine')||'kokoro';
      ttsProviderSel.value=savedProvider;
      _loadTtsProviders().then(_applyProviderUI);
      ttsProviderSel.onchange=function(){
    ...
    if(kokoroVoiceSel){
      kokoroVoiceSel.onchange=function(){
        localStorage.setItem('hermes-kokoro-voice',this.value);
      };
    }

    if(luxProfileSel){
      luxProfileSel.onchange=function(){
        localStorage.setItem('hermes-luxtts-profile-id',this.value);
      };
    }
    ...
    if(ttsRateSlider){
      const savedRate=localStorage.getItem('eburon-tts-rate');
      ttsRateSlider.value=savedRate||'1';
      if(ttsRateValue) ttsRateValue.textContent=parseFloat(ttsRateSlider.value).toFixed(1)+'x';
      ttsRateSlider.oninput=function(){if(ttsRateValue)ttsRateValue.textContent=parseFloat(this.value).toFixed(1)+'x';localStorage.setItem('eburon-tts-rate',this.value);};
    }
    const ttsPitchSlider=$('settingsTtsPitch');
    const ttsPitchValue=$('settingsTtsPitchValue');
    if(ttsPitchSlider){
      const savedPitch=localStorage.getItem('eburon-tts-pitch');
      ttsPitchSlider.value=savedPitch||'1';
      if(ttsPitchValue) ttsPitchValue.textContent=parseFloat(ttsPitchSlider.value).toFixed(1);
      ttsPitchSlider.oninput=function(){if(ttsPitchValue)ttsPitchValue.textContent=parseFloat(this.value).toFixed(1);localStorage.setItem('eburon-tts-pitch',this.value);};
    }
    const notifCb=$('settingsNotificationsEnabled');
    ...
    _syncEburonPanelSessionActions();
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  _settingsFontSizeOnOpen=fontSize||localStorage.getItem('eburon-font-size')||'default';
  const bar=$('settingsUnsavedBar');
  if(bar) bar.style.display='none';
  _settingsEburonDefaultModelOnOpen=body.default_model||_settingsEburonDefaultModelOnOpen||'';
...
async function saveSettings(andClose){
  ...
  const modelChanged=(model||'')!==(_settingsEburonDefaultModelOnOpen||'');
  ...
  const fontSize=($('settingsFontSize')||{}).value||localStorage.getItem('eburon-font-size')||'default';
