...
function _syncWorkspacePanelInlineWidth(){
  ...
  const saved = localStorage.getItem('eburon-panel-w');
  if(!saved) return;
...
function _setWorkspacePanelMode(mode){
  ...
  localStorage.setItem('eburon-webui-workspace-panel', open ? 'open' : 'closed');
...
(function(){
  ...
  const hasTTS=!!('speechSynthesis' in window)||!!window.HermesLuxTTS;
  ...
  function _voiceModePrefEnabled(){
    try{ return localStorage.getItem('eburon-voice-mode-button')==='true'; }
  ...
  function _speakResponse(){
    ...
    if(window.HermesLuxTTS&&typeof window.HermesLuxTTS.hadRecentStreamAudio==='function'&&window.HermesLuxTTS.hadRecentStreamAudio()){
      return;
    }

    if(window.HermesLuxTTS&&window.HermesLuxTTS.providerEnabled&&window.HermesLuxTTS.providerEnabled()){
      window.HermesLuxTTS.speak(clean,{
        onStart:()=>_setState('speaking'),
        onEnd:()=>{ if(_voiceModeActive) setTimeout(()=>_startListening(),500); },
      }).catch(()=>{ if(_voiceModeActive) setTimeout(()=>_startListening(),1000); });
      return;
    }
    ...
    const savedVoice=localStorage.getItem('eburon-tts-voice');
    const voices=speechSynthesis.getVoices();
    if(savedVoice&&voices.length){
      const match=voices.find(v=>v.name===savedVoice);
      if(match) utter.voice=match;
    }
    const savedRate=parseFloat(localStorage.getItem('eburon-tts-rate'));
    if(!isNaN(savedRate)) utter.rate=Math.min(2,Math.max(0.5,savedRate));
    const savedPitch=parseFloat(localStorage.getItem('eburon-tts-pitch'));
  ...
  window._voiceModeOnLuxTtsStart=function(){
    if(_voiceModeActive&&(_voiceModeState==='thinking'||_voiceModeState==='idle')){
      _setState('speaking');
    }
  };
  window._voiceModeOnLuxTtsComplete=function(){
...
$('modelSelect').onchange=async()=>{
  ...
  else try{localStorage.setItem('eburon-webui-model',modelState.model)}catch{}
...
(function(){
  ...
  window._initResizePanels = function(){
    ...
    initResize('sidebarResize',    sidebar,    'right', SIDEBAR_MIN, SIDEBAR_MAX, 'eburon-sidebar-w');
    initResize('rightpanelResize', rightpanel, 'left',  PANEL_MIN,   PANEL_MAX,   'eburon-panel-w');
  };
})();
...
function _syncThemeColorMeta(){
  try{
    const meta=document.getElementById('eburon-theme-color');
    if(!meta) return;
...
function _pickTheme(name){
  const currentSkin=localStorage.getItem('eburon-skin');
  const appearance=_normalizeAppearance(name,currentSkin);
  localStorage.setItem('eburon-theme',appearance.theme);
  localStorage.setItem('eburon-skin',appearance.skin);
...
function _pickSkin(name){
  const appearance=_normalizeAppearance(localStorage.getItem('eburon-theme'),name);
  localStorage.setItem('eburon-theme',appearance.theme);
  localStorage.setItem('eburon-skin',appearance.skin);
  _applyTheme(appearance.theme);
  _applySkin(appearance.skin);
...
function _pickFontSize(size){
  localStorage.setItem('eburon-font-size',size);
  _applyFontSize(size);
...
(async()=>{
  ...
  try{
    ...
    localStorage.setItem('eburon-skin',appearance.skin);
    _applySkin(appearance.skin);
    const fontSize=(s.font_size||localStorage.getItem('eburon-font-size')||'default');
    localStorage.setItem('eburon-font-size',fontSize);
  ...
  const _modelDropdownReady=populateModelDropdown().then(()=>{
    const savedState=(typeof _readPersistedModelState==='function')
      ...
      : (localStorage.getItem('eburon-webui-model')?{model:localStorage.getItem('eburon-webui-model'),model_provider:null}:null);
  ...
  if(saved){
    try{
      ...
      if(S.session && (S.session.message_count||0) === 0 && !_restoredInFlight){
        ...
        const _ephPanelPref=localStorage.getItem('eburon-webui-workspace-panel-pref')==='open'
          || localStorage.getItem('eburon-webui-workspace-panel')==='open';
      ...
      const panelPref=localStorage.getItem('eburon-webui-workspace-panel-pref')==='open'
        || localStorage.getItem('eburon-webui-workspace-panel')==='open';
  ...
  const _freshPanelPref=localStorage.getItem('eburon-webui-workspace-panel-pref')==='open'
    || localStorage.getItem('eburon-webui-workspace-panel')==='open';
