// Kokoro TTS client adapter for WebUI response audio.
(function(){
  const state={
  ...
  function _providerEnabled(){
    try{ return localStorage.getItem('hermes-kokoro-enabled')!=='false'; }
    catch(_){ return true; }
  }

  function _realtimeEnabled(){
    try{
      const explicit=localStorage.getItem('hermes-kokoro-realtime');
      return explicit===null?true:explicit==='true';
    }catch(_){ return true; }
  }

  function _voice(){
    try{ return localStorage.getItem('hermes-kokoro-voice')||''; }
    catch(_){ return ''; }
  }
  ...
  function _playBlob(blob, opts={}){
    ...
    return new Promise(resolve=>{
      ...
      audio.play().catch(err=>{
        if(token===state.stopToken) _toast('Kokoro TTS playback blocked: '+(err&&err.message?err.message:err));
        done();
      });
    });
  }
  ...
  function speak(text, opts={}){
    if(!_providerEnabled()) return Promise.reject(new Error('Kokoro TTS disabled'));
    ...
    return fetch(_url('api/tts/speak'),{
      ...
      if(opts.button) opts.button.dataset.speaking='0';
      _toast('Kokoro TTS failed: '+(err&&err.message?err.message:err),5000);
      throw err;
    });
  }
  ...
  function handleSseAudio(data, activeSid){
    ...
    state.chain=state.chain.catch(()=>{}).then(()=>_playBlob(blob,{
      onStart:()=>{
        if(!stream.started){
          stream.started=true;
          if(typeof window._voiceModeOnKokoroStart==='function'){
            window._voiceModeOnKokoroStart(stream.id);
          }
        }
      },
    }));
  ...
  function handleSseEnd(data){
    ...
    state.chain=state.chain.catch(()=>{}).then(()=>{
      if(stream.hadAudio&&typeof window._voiceModeOnKokoroComplete==='function'){
        window._voiceModeOnKokoroComplete(stream.id);
      }
  ...
  function handleSseError(data){
    const msg=data&&data.message?String(data.message):'Kokoro TTS stream failed';
