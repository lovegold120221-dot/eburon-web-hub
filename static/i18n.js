...
const LOCALES = {
  en: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" may not work with your configured provider (${p}). Send anyway, or run \`eburon model\` in your terminal to switch.`,
  ...
  untitled:'Untitled',
    no_personalities: 'No personalities found (add them to ~/.eburon/personalities/)',
  ...
  folder_add_as_space_title: 'Add as Space?',
    ...
    settings_section_preferences_meta: 'Defaults and UI behavior for Eburon Hub.',
    ...
    tab_dashboard: 'Eburon Hub Dashboard',
    ...
    settings_label_tts_provider: 'Voice Provider',
    settings_label_tts_voice_kokoro: 'Orbit Voice (Kokoro)',
    settings_label_tts_voice_luxtts: 'Eburon Voice (Jenny/LuxTTS)',
    provider_kokoro: 'Orbit Voice (Kokoro)',
    provider_luxtts: 'Eburon Voice (LuxTTS)',
    ...
    settings_desc_sync_insights: 'Mirrors WebUI token usage to state.db so eburon /insights includes browser session data. Off by default.',
    ...
    providers_oauth_config_yaml_hint: 'Token configured via config.yaml. To update, edit the providers section in your config.yaml or run eburon auth.',
    providers_oauth_not_configured_hint: 'Not authenticated. Run eburon auth in the terminal to configure this provider.',
    providers_save: 'Save',
    ...
    onboarding_title: 'Welcome to Eburon Hub',
    ...
    onboarding_step_system_desc: 'Verify Beatrice Agent and config visibility.',
    ...
    onboarding_notice_system_ready: 'Beatrice Agent looks reachable from the Web UI.',
    onboarding_notice_system_unavailable: 'Beatrice Agent is not fully available yet. Bootstrap can install it, but provider setup may still require a terminal.',
    onboarding_check_agent: 'Beatrice Agent',
    ...
    onboarding_oauth_provider_not_ready_body: 'This instance is configured to use <strong>{provider}</strong>, which uses OAuth rather than an API key. Run <code>eburon auth</code> or <code>eburon model</code> in a terminal to authenticate, then reload the Web UI.',
  ...
  ja: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" は設定されたプロバイダ (${p}) で動作しない可能性があります。このまま送信するか、ターミナルで \`eburon model\` を実行して切り替えてください。`,
  ...
  untitled:'無題',
    no_personalities: 'パーソナリティが見つかりません (~/.eburon/personalities/ に追加してください)',
  ...
  folder_add_as_space_title: 'スペースとして追加?',
    ...
    settings_section_preferences_meta: 'Eburon Hub のデフォルトと UI 動作。',
    ...
    settings_desc_sync_insights: 'WebUI のトークン使用量を state.db にミラーし、eburon /insights にブラウザセッションのデータを含めます。デフォルトはオフ。',
    ...
    providers_oauth_config_yaml_hint: 'config.yaml でトークンが設定されています。更新するには config.yaml の providers セクションを編集するか eburon auth を実行してください。',
    providers_oauth_not_configured_hint: '未認証です。ターミナルで eburon auth を実行してこのプロバイダを設定してください。',
    providers_save: '保存',
    providers_remove: '削除',
    ...
    onboarding_title: 'Eburon Hub へようこそ',
    ...
    onboarding_step_system_desc: 'Beatrice Agent と設定の可視性を確認します。',
    ...
    onboarding_notice_system_ready: 'Web UI から Beatrice Agent へ到達できそうです。',
    onboarding_notice_system_unavailable: 'Beatrice Agent はまだ完全には利用できません。Bootstrap でインストールできますが、プロバイダ設定にはターミナルが必要な場合があります。',
    onboarding_check_agent: 'Beatrice Agent',
    ...
    onboarding_oauth_provider_not_ready_body: 'このインスタンスは <strong>{provider}</strong> を使用しますが、これは APIキー ではなく OAuth を使用します。ターミナルで <code>eburon auth</code> または <code>eburon model</code> を実行して認証し、Web UI を再読み込みしてください。',
  ...
  ru: {
    ...
    `"${m}" может не работать с вашим настроенным провайдером (${p}). Всё равно отправить или запустите \`eburon model\` в терминале, чтобы переключиться.`,
    ...
    no_personalities: 'Личности не найдены (добавьте их в ~/.eburon/personalities/)',
  ...
  folder_add_as_space_title: 'Добавить как пространство?',
    ...
    onboarding_title: 'Добро пожаловать в Eburon Hub',
    ...
    onboarding_step_system_desc: 'Проверить Beatrice Agent и видимость конфигурации.',
    ...
    onboarding_notice_system_ready: 'Beatrice Agent, похоже, доступен из Web UI.',
    onboarding_notice_system_unavailable: 'Beatrice Agent ещё не полностью доступен. Bootstrap может установить его, но для настройки провайдера всё ещё может понадобиться терминал.',
    onboarding_check_agent: 'Beatrice Agent',
    ...
    onboarding_oauth_provider_not_ready_body: 'Этот экземпляр настроен на использование <strong>{provider}</strong>, который работает через OAuth, а не через API-ключ. Запустите <code>eburon auth</code> или <code>eburon model</code> в терминале, чтобы пройти авторизацию, затем обновите Web UI.',
  ...
  es: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" puede no funcionar con tu proveedor configurado (${p}). Envía de todas formas, o ejecuta \`eburon model\` en la terminal para cambiar.`,
    ...
    no_personalities: 'No se encontraron personalidades (añádelas a ~/.eburon/personalities/)',
  ...
  folder_add_as_space_title: '¿Añadir como espacio?',
    ...
    settings_desc_sync_insights: 'Refleja el uso de tokens de la WebUI en state.db para que eburon /insights incluya datos de sesiones del navegador. Desactivado por defecto.',
    ...
    onboarding_title: 'Bienvenido a Eburon Hub',
    ...
    onboarding_step_system_desc: 'Verifica Beatrice Agent y la visibilidad de la configuración.',
    ...
    onboarding_notice_system_ready: 'Parece que Beatrice Agent está accesible desde la Web UI.',
    onboarding_notice_system_unavailable: 'Beatrice Agent todavía no está totalmente disponible. Bootstrap puede instalarlo, pero la configuración del proveedor quizá aún requiera una terminal.',
    onboarding_check_agent: 'Beatrice Agent',
    ...
    onboarding_oauth_provider_not_ready_body: 'Esta instancia está configurada para usar <strong>{provider}</strong>, que utiliza OAuth en lugar de una clave API. Ejecuta <code>eburon auth</code> o <code>eburon model</code> en una terminal para autenticarte y recarga la interfaz web.',
  ...
  de: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" funktioniert möglicherweise nicht mit Ihrem konfigurierten Provider (${p}). Trotzdem senden, oder \`eburon model\` im Terminal ausführen.`,
    ...
    no_personalities: 'Keine Persönlichkeiten gefunden (füge sie in ~/.eburon/personalities/ hinzu)',
  ...
  folder_add_as_space_title: 'Als Bereich hinzufügen?',
    ...
    settings_desc_sync_insights: 'Spiegelt den WebUI-Token-Verbrauch in die state.db, sodass eburon /insights Browser-Sitzungsdaten enthält. Standardmäßig aus.',
    ...
    onboarding_step_system_desc: 'Beatrice Agent muss installiert und erreichbar sein.',
    ...
    onboarding_notice_system_unavailable: 'Agent nicht erreichbar. Bitte starten Sie Beatrice Agent.',
  ...
  zh: {
    ...
    provider_mismatch_warning: (m,p)=>`\"${m}\" \u53ef\u80fd\u65e0\u6cd5\u5728\u5f53\u524d\u914d\u7f6e\u7684\u63d0\u4f9b\u5546 (${p}) \u4e0b\u5de5\u4f5c\u3002\u76f4\u63a5\u53d1\u9001\uff0c\u6216\u5728\u7ec8\u7aef\u8fd0\u884c \`eburon model\` \u5207\u6362\u3002`,
    ...
    workspace_hidden_files_visible_title: 'Hidden files are visible — click for options',
    workspace_options: 'Workspace options',
    no_personalities: '\u6ca1\u6709\u627e\u5230\u4eba\u8bbe\uff08\u53ef\u6dfb\u52a0\u5230 ~/.eburon/personalities/\uff09',
  ...
  folder_add_as_space_title: '\u6dfb\u52a0\u4e3a\u5de5\u4f5c\u533a\uff1f',
    ...
    settings_desc_sync_insights: '将 WebUI token 使用情况同步到 state.db，使 eburon /insights 包含浏览器会话数据。默认关闭。',
    ...
    onboarding_title: '欢迎使用 Eburon Hub',
    ...
    onboarding_step_system_desc: '验证 Beatrice Agent 与配置可见性。',
    ...
    onboarding_notice_system_ready: 'Beatrice Agent 看起来可从 Web UI 访问。',
    onboarding_notice_system_unavailable: 'Beatrice Agent 尚未完全可用。Bootstrap 可以安装它，但提供商设置可能仍需要终端。',
    onboarding_check_agent: 'Beatrice Agent',
    ...
    onboarding_oauth_provider_not_ready_body: '此实例已配置为使用 <strong>{provider}</strong>，该提供商使用 OAuth 而非 API key。请在终端运行 <code>eburon auth</code> 或 <code>eburon model</code> 完成认证，然后重新加载 Web UI。',
  ...
  'zh-Hant': {
    ...
    provider_mismatch_warning: (m,p)=>`\"${m}\" \u53ef\u80fd\u7121\u6cd5\u5728\u7576\u524d\u914d\u7f6e\u7684\u63d0\u4f9b\u8005 (${p}) \u4e0b\u904b\u4f5c\u3002\u5c1a\u9001\uff0c\u6216\u5728\u7d42\u7aef\u57f7\u884c \`eburon model\` \u5207\u63db\u3002`,
    ...
    workspace_hidden_files_visible_title: 'Hidden files are visible — click for options',
    workspace_options: 'Workspace options',
    no_personalities: '\u6c92\u6709\u627e\u5230\u4eba\u8a2d\uff08\u53ef\u6dfb\u52a0\u5230 ~/.eburon/personalities/\uff09',
  ...
  folder_add_as_space_title: '\u65b0\u589e\u70ba\u5de5\u4f5c\u5340\uff1f',
    ...
    settings_section_preferences_meta: 'Eburon Hub 的預設值與介面行為。',
    ...
    settings_desc_sync_insights: '將 WebUI token 使用情況同步到 state.db，使 eburon /insights 包含瀏覽器會話數據。預設未啟用。',
    ...
    onboarding_notice_system_ready: 'Beatrice Agent \u53ef\u5f9e Web UI \u5b58\u53d6\u3002',
    onboarding_notice_system_unavailable: 'Beatrice Agent \u5c1a\u672a\u5b8c\u5168\u53ef\u7528\u3002\u5f15\u5c0e\u7a0b\u5f0f\u53ef\u80fd\u5b89\u88dd\u5b83\uff0c\u4f46\u63d0\u4f9b\u8005\u8a2d\u5b9a\u53ef\u80fd\u4ecd\u9700\u7d42\u7aef\u6a5f\u3002',
    onboarding_notice_workspace: '\u9019\u4e9b\u503c\u4f7f\u7528\u8207\u4e00\u822c\u61c9\u7528\u7a0b\u5f0f\u76f8\u540c\u7684\u8a2d\u5b9a API\u3002',
    onboarding_oauth_provider_not_ready_body: '\u6b64\u5be6\u4f8b\u8a2d\u5b9a\u4f7f\u7528 <strong>{provider}</strong>\uff0c\u5b83\u4f7f\u7528 OAuth \u800c\u975e API \u91d1\u9470\u3002\u8acb\u5728\u7d42\u7aef\u6a5f\u4e2d\u57f7\u884c <code>eburon auth</code> \u6216 <code>eburon model</code> \u9032\u884c\u6388\u6b0a\uff0c\u7136\u5f8c\u91cd\u65b0\u6574\u7406 Web UI\u3002',
    ...
    providers_oauth_config_yaml_hint: '\u5df2\u900f\u904e config.yaml \u8a2d\u5b9a\u6b0a\u8b49\u3002\u8981\u66f4\u65b0\uff0c\u8acb\u7de8\u8f2f config.yaml \u4e2d\u7684 providers \u5340\u6bb5\uff0c\u6216\u57f7\u884c eburon auth\u3002',
    providers_oauth_not_configured_hint: '\u5c1a\u672a\u9a57\u8b49\u3002\u8acb\u5728\u7d42\u7aef\u6a5f\u57f7\u884c eburon auth \u4ee5\u8a2d\u5b9a\u6b64\u63d0\u4f9b\u8005\u3002',
    providers_remove: '\u79fb\u9664',
  ...
  pt: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" pode não funcionar com seu provedor configurado (${p}). Enviar assim mesmo, ou execute \`eburon model\` no terminal para trocar.`,
    ...
    no_personalities: 'Nenhuma personalidade encontrada (adicione em ~/.eburon/personalities/)',
    ...
    settings_section_preferences_meta: 'Padrões e comportamento UI do Eburon Hub.',
    ...
    providers_oauth_config_yaml_hint: 'Token configurado via config.yaml. Para atualizar, edite config.yaml ou rode eburon auth.',
    providers_oauth_not_configured_hint: 'Não autenticado. Rode eburon auth no terminal.',
    ...
    onboarding_title: 'Bem-vindo ao Eburon Hub',
    ...
    onboarding_step_system_desc: 'Verificar Beatrice Agent e visibilidade da config.',
    ...
    onboarding_notice_system_ready: 'Beatrice Agent parece acessível pela Web UI.',
    onboarding_notice_system_unavailable: 'Beatrice Agent não está totalmente disponível. Bootstrap pode instalar, mas setup do provedor pode requerer terminal.',
    onboarding_check_agent: 'Beatrice Agent',
    ...
    onboarding_oauth_provider_not_ready_body: 'Esta instância usa <strong>{provider}</strong> com OAuth. Rode `eburon auth` no terminal.',
  ...
  ko: {
    ...
    settings_section_preferences_meta: 'Eburon Hub의 기본값과 UI 동작.',
    ...
    settings_desc_sync_insights: 'WebUI 토큰 사용량을 state.db에 반영하여 eburon /insights에 브라우저 세션 데이터가 포함되도록 합니다. 기본값은 꺼짐입니다.',
    ...
    onboarding_title: 'Eburon Hub에 오신 것을 환영합니다',
