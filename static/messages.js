...
async function send(){
  ...
  try{
    const startData=await api('/api/chat/start',{method:'POST',body:JSON.stringify({
      ...
      tts:(function(){
        const engine=localStorage.getItem('hermes-tts-engine')||'kokoro';
        if(engine==='luxtts'&&window.HermesLuxTTS&&typeof window.HermesLuxTTS.buildStartOptions==='function'){
          return window.HermesLuxTTS.buildStartOptions();
        }
        if(window.HermesKokoroTTS&&typeof window.HermesKokoroTTS.buildStartOptions==='function'){
          return window.HermesKokoroTTS.buildStartOptions();
        }
        return undefined;
      })()
    })});
    ...
    if(window.HermesLuxTTS&&typeof window.HermesLuxTTS.noteStreamStarted==='function'){
      window.HermesLuxTTS.noteStreamStarted(streamId, activeSid);
    }
    if(window.HermesKokoroTTS&&typeof window.HermesKokoroTTS.noteStreamStarted==='function'){
      window.HermesKokoroTTS.noteStreamStarted(streamId, activeSid);
    }
...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('tts_audio',e=>{
      let d={};
      try{ d=JSON.parse(e.data||'{}')||{}; }catch(_){ d={}; }
      if(d.session_id&&d.session_id!==activeSid) return;
      const eng=d.engine||'';
      if(eng==='luxtts'&&window.HermesLuxTTS&&typeof window.HermesLuxTTS.handleSseAudio==='function'){
        window.HermesLuxTTS.handleSseAudio(d, activeSid);
      } else if(eng==='kokoro'&&window.HermesKokoroTTS&&typeof window.HermesKokoroTTS.handleSseAudio==='function'){
        window.HermesKokoroTTS.handleSseAudio(d, activeSid);
      }
    });

    source.addEventListener('tts_end',e=>{
      let d={};
      try{ d=JSON.parse(e.data||'{}')||{}; }catch(_){ d={}; }
      if(d.session_id&&d.session_id!==activeSid) return;
      const eng=d.engine||'';
      if(eng==='luxtts'&&window.HermesLuxTTS&&typeof window.HermesLuxTTS.handleSseEnd==='function'){
        window.HermesLuxTTS.handleSseEnd(d);
      } else if(eng==='kokoro'&&window.HermesKokoroTTS&&typeof window.HermesKokoroTTS.handleSseEnd==='function'){
        window.HermesKokoroTTS.handleSseEnd(d);
      }
    });

    source.addEventListener('tts_error',e=>{
      let d={};
      try{ d=JSON.parse(e.data||'{}')||{}; }catch(_){ d={}; }
      if(d.session_id&&d.session_id!==activeSid) return;
      const eng=d.engine||'';
      if(eng==='luxtts'&&window.HermesLuxTTS&&typeof window.HermesLuxTTS.handleSseError==='function'){
        window.HermesLuxTTS.handleSseError(d);
      } else if(eng==='kokoro'&&window.HermesKokoroTTS&&typeof window.HermesKokoroTTS.handleSseError==='function'){
        window.HermesKokoroTTS.handleSseError(d);
      }
    });
    ...
    source.addEventListener('done',e=>{
      ...
      if(isActiveSession){
        ...
        if(typeof autoReadLastAssistant==='function') setTimeout(()=>autoReadLastAssistant(streamId), 300);
      }
