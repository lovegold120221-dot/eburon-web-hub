...
function openEburonHubDashboard(event){
...
function speakMessage(btn){
  ...
  // Prefer LuxTTS if available
  const luxAvailable=window.HermesLuxTTS&&window.HermesLuxTTS.providerEnabled&&window.HermesLuxTTS.providerEnabled();
  if(luxAvailable){
    window.HermesLuxTTS.speak(clean,{button:btn}).catch(()=>{});
    return;
  }
...
function stopTTS(){
  if(window.HermesLuxTTS&&typeof window.HermesLuxTTS.stop==='function'){
    window.HermesLuxTTS.stop();
  }
  if('speechSynthesis' in window){
...
function autoReadLastAssistant(streamId){
  if(window.HermesLuxTTS&&typeof window.HermesLuxTTS.hadRecentStreamAudio==='function'&&window.HermesLuxTTS.hadRecentStreamAudio(streamId)){
    return;
  }
  ...
  if(window.HermesLuxTTS&&window.HermesLuxTTS.providerEnabled&&window.HermesLuxTTS.providerEnabled()){
    window.HermesLuxTTS.speak(clean).catch(()=>{});
    return;
  }
...
function syncTopbar(){
  if(!S.session){
    ...
    if(typeof _syncEburonPanelSessionActions==='function') _syncEburonPanelSessionActions();
    else {
