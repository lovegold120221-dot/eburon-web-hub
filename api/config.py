"""
Eburon Hub -- Shared configuration, constants, and global state.
...
HOST = os.getenv("EBURON_HUB_HOST", "127.0.0.1")
PORT = int(os.getenv("EBURON_HUB_PORT", "8787"))

# ── TLS/HTTPS config (optional, env-overridable) ────────────────────────────
TLS_CERT = os.getenv("EBURON_HUB_TLS_CERT", "").strip() or None
TLS_KEY = os.getenv("EBURON_HUB_TLS_KEY", "").strip() or None
TLS_ENABLED = TLS_CERT is not None and TLS_KEY is not None

# ── State directory (env-overridable, never inside repo) ──────────────────────
STATE_DIR = (
    Path(os.getenv("EBURON_HUB_STATE_DIR", str(HOME / ".eburon" / "webui")))
    .expanduser()
    .resolve()
)
...
def _discover_agent_dir() -> Path:
    ...
    Priority:
      1. EBURON_HUB_AGENT_DIR env var  -- explicit override always wins
      2. EBURON_HOME / hermes-agent      -- e.g. ~/.eburon/hermes-agent
      3. Sibling of this repo            -- ../hermes-agent
      4. Parent of this repo             -- ../../hermes-agent (nested layout)
      5. Common install paths            -- ~/.eburon/hermes-agent (again as fallback)
      6. HOME / hermes-agent             -- ~/hermes-agent (simple flat layout)
    """
    candidates = []

    # 1. Explicit env var
    if os.getenv("EBURON_HUB_AGENT_DIR"):
        candidates.append(
            Path(os.getenv("EBURON_HUB_AGENT_DIR")).expanduser().resolve()
        )

    # 2. EBURON_HOME / hermes-agent
    hermes_home = os.getenv("EBURON_HOME", str(HOME / ".eburon"))
    ...
    # 5. ~/.eburon/hermes-agent (explicit common path)
    candidates.append(HOME / ".eburon" / "hermes-agent")
...
def _discover_python(agent_dir: Path) -> str:
    ...
    Priority:
      1. EBURON_HUB_PYTHON env var
      2. Agent venv at <agent_dir>/venv/bin/python
      3. Local .venv inside this repo
      4. System python3
    """
    if os.getenv("EBURON_HUB_PYTHON"):
        return os.getenv("EBURON_HUB_PYTHON")
...
def _cfg_has_in_memory_overrides() -> bool:
    ...
    """
    if (
        _cfg_fingerprint is not None
        and _fingerprint_config(_cfg_cache) != _cfg_fingerprint
    ):
        return True
...
def _get_config_path() -> Path:
    ...
    try:
        ...
        return HOME / ".eburon" / "config.yaml"
...
def _workspace_candidates(raw: str | Path | None = None) -> list[Path]:
    ...
    add(raw)
    if os.getenv("EBURON_HUB_DEFAULT_WORKSPACE"):
        add(os.getenv("EBURON_HUB_DEFAULT_WORKSPACE"))
...
def resolve_default_workspace(raw: str | Path | None = None) -> Path:
    ...
    raise RuntimeError(
        ...
        "Set EBURON_HUB_DEFAULT_WORKSPACE to a writable path."
    )
...
def _discover_default_workspace() -> Path:
    ...
    Resolve the default workspace in order:
      1. EBURON_HUB_DEFAULT_WORKSPACE env var
...
def print_startup_config() -> None:
    ...
    if not _EBURON_FOUND:
        print(
            f"{err}  Could not find the Hermes agent directory.\n"
            "      The server will start but agent features will not work.\n"
            "\n"
            "      To fix, set one of:\n"
            "        export EBURON_HUB_AGENT_DIR=/path/to/hermes-agent\n"
            "        export EBURON_HOME=/path/to/.hermes\n"
            "\n"
...
def verify_eburon_imports() -> tuple:
    """
...
def _resolve_cli_toolsets(cfg=None):
    ...
    try:
        ...
        # Use api_server platform for WebUI to get full tool access
        return list(_get_platform_tools(cfg, "api_server"))
    except Exception:
        # Fallback: read raw list from config (MCP toolsets will be missing)
        return cfg.get("platform_toolsets", {}).get(
            "api_server", cfg.get("platform_toolsets", {}).get("cli", _DEFAULT_TOOLSETS)
        )
...
_PROVIDER_MODELS = {
    ...
    "nous": [
        {
            "id": "@nous:anthropic/claude-opus-4.6",
            "label": "Claude Opus 4.6 (via Nous)",
        },
        {
            "id": "@nous:anthropic/claude-sonnet-4.6",
            "label": "Claude Sonnet 4.6 (via Nous)",
        },
        {"id": "@nous:openai/gpt-5.4-mini", "label": "GPT-5.4 Mini (via Nous)"},
        {
            "id": "@nous:google/gemini-3.1-pro-preview",
            "label": "Gemini 3.1 Pro Preview (via Nous)",
        },
    ],
    "zai": [
...
def resolve_model_provider(model_id: str) -> tuple:
    ...
    if model_id.startswith("@") and ":" in model_id:
        ...
        if provider_hint.startswith("custom:") and provider_hint.count(":") >= 2:
            ...
            provider_hint not in _PROVIDER_MODELS
    ...
    if "/" in model_id:
        ...
        if config_base_url:
            ...
            if str(
                config_provider or ""
            ).lower() in _LOCAL_SERVER_PROVIDERS or _base_url_points_at_local_server(
                config_base_url
            ):
                return model_id, config_provider, config_base_url
...
_cache_build_cv = threading.Condition(
    _available_models_cache_lock
)  # shares underlying RLock so notify_all() is safe inside with _available_models_cache_lock
...
_CREDENTIAL_POOL_CACHE: dict[
    str, tuple[float, "CredentialPool"]
] = {}  # pid -> (ts, pool)
_provider_models_invalidated_ts: dict[
    str, float
] = {}  # provider_id -> timestamp of last invalidation
...
# EBURON_HUB_STATE_DIR / port) has its own file and test runs never
...
def _get_label_for_model(model_id: str, existing_groups: list) -> str:
    ...
    return " ".join(
        w.upper()
        if (len(w) <= 3 and w.replace(".", "").isalnum() and not w.isdigit())
        else w.capitalize()
...
def _models_from_live_provider_ids(provider_id: str, live_ids: list[str]) -> list[dict]:
    ...
    formatter = (
        _format_ollama_label if provider_id in ("ollama", "ollama-cloud") else None
    )
    models: list[dict] = []
    seen: set[str] = set()
    for mid in live_ids:
...
def _read_visible_codex_cache_model_ids() -> list[str]:
    ...
    """
    codex_home = Path(
        os.getenv("CODEX_HOME", "").strip() or (HOME / ".codex")
    ).expanduser()
...
def get_available_models() -> dict:
    ...
    Discovery order:
      ...
      2. Check for known API keys in env or ~/.eburon/.env
    ...
    def _build_available_models_uncached() -> dict:
        ...
        def _build_configured_model_badges() -> dict[str, dict[str, str]]:
            ...
            for entry in configured_entries:
                ...
                match_id = None
                exact_match = next(
                    (option_lookup[c] for c in raw_candidates if c in option_lookup),
                    None,
                )
                    ...
                    for candidate in raw_candidates:
                        ...
                        provider_match = next(
                            (
                                m
                                for m in matches
                                if option_provider_lookup.get(m) == provider
                            ),
                            None,
                        )
        ...
        try:
            ...
            for _p in _lap():
                ...
                try:
                    ...
                    logger.debug(
                        "Failed to get key source for provider %s",
                        _p.get("id", "unknown"),
                    )
        ...
        if not _hermes_auth_used:
            try:
                ...
                hermes_env_path = HOME / ".eburon" / ".env"
            env_keys = {}
        ...
        if isinstance(_cfg_providers, dict):
            for _pid_key in _cfg_providers:
                ...
                if (
                    ...
                    or _canonical in _cfg_providers
        ...
        def _configured_provider_for_base_url(base_url: object) -> str:
            ...
            if isinstance(model_cfg, dict):
                model_base_url = _normalize_base_url_for_match(
            ...
            if isinstance(custom_providers_cfg, list):
                for entry in custom_providers_cfg:
                    if not isinstance(entry, dict):
                        continue
                    entry_base_url = _normalize_base_url_for_match(
                        entry.get("base_url")
                    )
                    if entry_base_url != target:
                        continue
        ...
        if cfg_base_url:
            try:
                ...
                if cfg_base_url:
                    _base_parsed = urlparse(
                        cfg_base_url
                        if "://" in cfg_base_url
                        else f"http://{cfg_base_url}"
                    )
                ...
                if isinstance(_custom_providers_cfg, list):
                    for _cp in _custom_providers_cfg:
                        ...
                        if _cp_base:
                            _cp_parsed = urlparse(
                                _cp_base if "://" in _cp_base else f"http://{_cp_base}"
                            )
                ...
                for model in models_list:
                    ...
                    model_name = (
                        model.get("name", "") or model.get("model", "") or model_id
                    )
                    if model_id and model_name:
                        label = (
                            _format_ollama_label(model_id)
                        ...
                        auto_detected_models_by_provider.setdefault(
                            provider_key, []
                        ).append(auto_model)
            ...
            for _cp in _custom_providers_cfg:
                ...
                if isinstance(_cp_models_dict, dict):
                    for _m_id in _cp_models_dict:
                        if (
                            isinstance(_m_id, str)
                            and _m_id.strip()
                            and _m_id not in _cp_model_ids
                        ):
                ...
                for _cp_model in _cp_model_ids:
                    if _cp_model and _cp_model not in _seen_custom_ids:
                        ...
                        if _slug:
                            ...
                            if (
                                active_provider != _slug
                                and not _cp_option_id.startswith("@")
                            ):
        ...
        if (
            active_provider
            and active_provider != "custom"
            and not _has_custom_providers
        ):
        ...
        if _base_matched_named_slug and _named_custom_slugs:
            for _pid in list(detected_providers):
                ...
                if (
                    ...
                    and _pid_norm not in _named_custom_slugs
                ):
                    detected_providers.discard(_pid)

        # Filter providers if providers.only_configured is set
        providers_cfg = cfg.get("providers", {})
        only_show_configured = (
            providers_cfg.get("only_configured", False)
            if isinstance(providers_cfg, dict)
            else False
        )
        ...
        # Auto-probe local Ollama even when no base_url is explicitly configured.
        # This lets self-hosted Ollama appear in the picker without requiring
        # a manual config.yaml edit — matching the keyless auto-detect UX of
        # the onboarding wizard.
        if "ollama" not in detected_providers:
            try:
                import urllib.request as _ureq

                _ollama_req = _ureq.Request(
                    "http://localhost:11434/v1/models", method="GET"
                )
                _ollama_req.add_header("User-Agent", "OpenAI/Python 1.0")
                with _ureq.urlopen(_ollama_req, timeout=3) as _ollama_resp:
                    _ollama_data = json.loads(_ollama_resp.read().decode("utf-8"))
                _ollama_models = []
                if "data" in _ollama_data and isinstance(_ollama_data["data"], list):
                    _ollama_models = _ollama_data["data"]
                elif "models" in _ollama_data and isinstance(
                    _ollama_data["models"], list
                ):
                    _ollama_models = _ollama_data["models"]
                for _m in _ollama_models:
                    if isinstance(_m, dict):
                        ...
                        if _mid:
                            _label = _format_ollama_label(_mid)
                            auto_detected_models.append({"id": _mid, "label": _label})
                            auto_detected_models_by_provider.setdefault(
                                "ollama", []
                            ).append({"id": _mid, "label": _label})
                if auto_detected_models_by_provider.get("ollama"):
                    detected_providers.add("ollama")
            except Exception:
                pass
        ...
        if detected_providers:
            for pid in sorted(
                detected_providers,
                key=lambda p: (
                    0 if p == "ollama" else 1 if p == "ollama-cloud" else 2,
                    p,
                ),
            ):
                ...
                if pid == "openrouter":
                    ...
                    try:
                        ...
                        logger.warning(
                            "Failed to load OpenRouter curated catalog from hermes_cli"
                        )
                    ...
                    try:
                        ...
                        for _item in _payload.get("data", []) or []:
                            ...
                            try:
                                _is_free = (
                                    ...
                                    and float(_pricing.get("completion", "0") or "0")
                                    == 0
                                )
                            ...
                            if not _is_free:
                                continue
                            _name = str(_item.get("name") or "").strip() or _mid
                        ...
                        logger.debug(
                            "OpenRouter free-tier live fetch unavailable; using fallback"
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            "Failed to load Ollama Cloud models from hermes_cli"
                        )

                    if raw_models:
                        models = _apply_provider_prefix(
                            raw_models, pid, active_provider
                        )
                        groups.append(
                            {
                    ...
                    try:
                        ...
                        codex_ids = [
                            mid
                            for mid in (_provider_model_ids("openai-codex") or [])
                            if mid
                        ]
                    except Exception:
                        logger.warning(
                            "Failed to load OpenAI Codex models from hermes_cli"
                        )
                    ...
                    if not raw_models:
                        raw_models = copy.deepcopy(
                            _PROVIDER_MODELS.get("openai-codex", [])
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            "Failed to load Nous Portal models from hermes_cli"
                        )
                        live_ids = []
                    ...
                    if raw_models:
                        ...
                        extras = (
                            _apply_provider_prefix(extra_models, pid, active_provider)
                            if extra_models
                            else []
                        )
                        group_entry = {
                    ...
                    if isinstance(provider_cfg, dict) and "models" in provider_cfg:
                        ...
                        if isinstance(cfg_models, dict):
                            raw_models = [
                                {"id": k, "label": k} for k in cfg_models.keys()
                            ]
        ...
        if default_model:
            ...
            _looks_like_provider_id = (
                str(default_model).strip().lower().replace("_", "-")
                in _PROVIDER_DISPLAY
            ...
            if _looks_like_provider_id:
                ...
                all_ids_norm = {
                    _norm_model_id(m["id"]) for g in groups for m in g.get("models", [])
                }
                if _norm_model_id(default_model) not in all_ids_norm:
                    label = _get_label_for_model(default_model, groups)
                    target_display = (
                        _PROVIDER_DISPLAY.get(
                    ...
                    for g in groups:
                        if (
                            target_display
                            and g.get("provider", "").lower() == target_display
                        ):
        ...
        groups = [
            g
            for g in groups
            if g.get("models") or (g.get("provider_id") or "").startswith("custom:")
        ]
    ...
    with _available_models_cache_lock:
        ...
        if should_wait:
            _cache_build_cv.wait_for(
                lambda: (
                    not _cache_build_in_progress and _available_models_cache is not None
                ),
                timeout=60,
            )
...
SESSION_AGENT_CACHE_MAX = (
    50  # Maximum cached agents (each holds full conversation history)
)
...
# Exception: if EBURON_HUB_DEFAULT_WORKSPACE is explicitly set in the
...
if _settings_file_exists:
    if not os.getenv("EBURON_HUB_DEFAULT_WORKSPACE"):
        DEFAULT_WORKSPACE = resolve_default_workspace(
            _startup_settings.get("default_workspace")
        )
    _startup_settings.pop(
        "default_model", None
    )  # always drop stale value; model comes from config.yaml
