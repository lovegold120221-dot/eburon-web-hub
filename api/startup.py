...
def fix_credential_permissions() -> None:
    """Ensure sensitive files in EBURON_HOME have safe permissions.

    Respects:
      - HERMES_SKIP_CHMOD=1  → bypass entirely
      - EBURON_HOME_MODE     → group bits are allowed if set by the operator,
    ...
    declared_mode = None
    raw_mode = os.environ.get('EBURON_HOME_MODE', '').strip()
    if raw_mode:
        try:
    ...
    hermes_home = Path(os.environ.get('EBURON_HOME', str(Path.home() / '.hermes')))
...
def _agent_dir() -> Path | None:
    hermes_home = Path(os.environ.get('EBURON_HOME', str(Path.home() / '.hermes')))
    for raw in [os.environ.get('EBURON_HUB_AGENT_DIR', '').strip(), str(hermes_home / 'hermes-agent')]:
        if not raw:
            continue
...
def _trusted_agent_dir(agent_dir: Path) -> bool:
    ...
    the dir to be ~/.hermes/hermes-agent), so custom EBURON_HUB_AGENT_DIR
    paths work correctly when EBURON_HUB_AUTO_INSTALL=1 is set.
    """
    try:
        st = agent_dir.stat()
...
def auto_install_agent_deps() -> bool:
    enabled = os.environ.get('EBURON_HUB_AUTO_INSTALL', '').strip().lower() in ('1', 'true', 'yes')
    if not enabled:
        print('[!!] Auto-install disabled. Set EBURON_HUB_AUTO_INSTALL=1 to enable.', flush=True)
        return False
