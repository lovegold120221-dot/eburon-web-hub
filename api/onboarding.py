...
_SUPPORTED_PROVIDER_SETUPS = {
    ...
    "x-ai": {
        ...
        "models": list(
            _PROVIDER_MODELS.get("xai", []) or _PROVIDER_MODELS.get("x-ai", [])
        ),
        "category": "specialized",
    },
}
...
def _provider_api_key_present(
    ...
    # rewrite.
    for alias in (
        _SUPPORTED_PROVIDER_SETUPS.get(provider, {}).get("env_var_aliases", []) or []
    ):
...
def _provider_oauth_authenticated(provider: str, hermes_home: "Path") -> bool:
    ...
    """
    provider = (provider or "").strip().lower()
    provider = {"claude": "anthropic", "claude-code": "anthropic"}.get(
        provider, provider
    )
    if not provider:
        return False
...
def _status_from_runtime(cfg: dict, imports_ok: bool) -> dict:
        ...
        if provider in _SUPPORTED_PROVIDER_SETUPS:
            ...
            if meta.get("key_optional"):
                ...
                if meta.get("requires_base_url"):
                    ...
                    provider_ready = _provider_api_key_present(
                        provider, cfg, env_values
                    )
    ...
    chat_ready = bool(_EBURON_FOUND and imports_ok and provider_ready)

    if not _EBURON_FOUND or not imports_ok:
...
def get_onboarding_status() -> dict:
    ...
    cfg = get_config()
    imports_ok, missing, errors = verify_eburon_imports()
    runtime = _status_from_runtime(cfg, imports_ok)
    workspaces = load_workspaces()
    last_workspace = get_last_workspace()
    available_models = get_available_models()

    # EBURON_HUB_SKIP_ONBOARDING=1 lets hosting providers (e.g. Agent37) ship
    ...
    skip_env = os.environ.get("EBURON_HUB_SKIP_ONBOARDING", "").strip()
    ...
    return {
        ...
        "system": {
            "hermes_found": bool(_EBURON_FOUND),
...
def apply_onboarding_setup(body: dict) -> dict:
    ...
    if not api_key and not _provider_api_key_present(provider, cfg, env_values):
        ...
        # marker is present.
        oauth_ready = bool(
            provider_meta.get("oauth_provider")
        ) and _provider_oauth_authenticated(
