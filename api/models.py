...
def _get_profile_home(profile) -> Path:
    ...
    EBURON_HOME environment variable or ~/.hermes, expanding ~ correctly.
    """
    try:
        from api.profiles import get_hermes_home_for_profile
        return Path(get_hermes_home_for_profile(profile))
    except ImportError:
        return Path(os.environ.get('EBURON_HOME') or '~/.hermes').expanduser()
...
def _active_state_db_path() -> Path:
    """Return state.db for the active Hermes profile, degrading to EBURON_HOME."""
    try:
        from api.profiles import get_active_hermes_home
        hermes_home = Path(get_active_hermes_home()).expanduser().resolve()
    except Exception:
        hermes_home = Path(os.getenv('EBURON_HOME', str(HOME / '.hermes'))).expanduser().resolve()
...
def _default_claude_code_projects_dir() -> Path | None:
    ...
    override = os.getenv('EBURON_HUB_CLAUDE_PROJECTS_DIR')
    if override:
        return Path(override).expanduser()
    if os.getenv('EBURON_HUB_TEST_STATE_DIR'):
        return None
...
def get_cli_sessions() -> list:
    ...
    # Use the active WebUI profile's EBURON_HOME to find state.db.
    ...
    # EBURON_HOME (which is the server's launch profile, not necessarily the
