...
def _gateway_session_metadata_path():
    try:
        ...
        hermes_home = Path(os.getenv("EBURON_HOME", str(Path.home() / ".hermes"))).expanduser().resolve()
...
def _run_cron_job_in_profile_subprocess(job, execution_profile_home):
    ...
    cron.scheduler/cron.jobs still rely on process-global EBURON_HOME and module
...
def _allowed_public_origins() -> set[str]:
    """Parse EBURON_HUB_ALLOWED_ORIGINS env var (comma-separated) into a set.
    ...
    """
    raw = os.getenv('EBURON_HUB_ALLOWED_ORIGINS', '')
    result = set()
    for value in raw.split(','):
        ...
        if not (value.startswith('http://') or value.startswith('https://')):
            import sys
            print(
                f"[webui] WARNING: EBURON_HUB_ALLOWED_ORIGINS entry {value!r} is missing "
...
def _split_provider_qualified_model(model: str) -> tuple[str, str | None]:
    ...
    if not model.startswith("@") or ":" not in model:
        return model, None
    inner = model[1:]
    # For custom: provider hints, the named slug itself may contain ':' (e.g.
    # ollama-(localhost:11434)).  We must match the full slug before splitting
    # so that model names like 'beatrice:latest' survive intact.
    if inner.startswith("custom:"):
        from api.config import _named_custom_provider_slugs, cfg as _cfg
        for _slug in _named_custom_provider_slugs(_cfg):
            # _slug already includes the 'custom:' prefix (e.g. custom:ollama-(localhost:11434))
            _prefix = f"{_slug}:"
            if inner.startswith(_prefix):
                provider = _clean_session_model_provider(_slug)
                bare = inner[len(_prefix):].strip()
                if provider and bare:
                    return bare, provider
        # No named slug matched — the first ':' separates provider from model
...
def _handle_logs(handler, parsed) -> bool:
    ...
    try:
        ...
        hermes_home = Path(os.environ.get("EBURON_HOME") or (Path.home() / ".hermes")).expanduser()
...
def _llm_wiki_active_hermes_home() -> Path:
    try:
        ...
        return Path(os.getenv("EBURON_HOME", str(Path.home() / ".hermes"))).expanduser()
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/settings":
        ...
        settings["password_env_var"] = bool(
            os.getenv("EBURON_HUB_PASSWORD", "").strip()
        )
    ...
    if parsed.path.startswith("/voice-orb/") or parsed.path == "/voice-orb" or parsed.path == "/voice-orb/index.html":
        return _serve_voice_orb(handler, parsed)
    ...
    if parsed.path == "/api/tts/status":
        from api import luxtts as _luxtts
        from api import kokoro as _kokoro

        kokoro_payload = _kokoro.status_payload()
        luxtts_payload = _luxtts.status_payload()

        default_engine = "kokoro"
        if kokoro_payload.get("enabled") and kokoro_payload.get("available"):
            default_engine = "kokoro"
        elif luxtts_payload.get("enabled") and luxtts_payload.get("healthy"):
        ...
        return j(handler, {
            ...
            "providers": {
                "kokoro": kokoro_payload,
                "luxtts": luxtts_payload,
            },
        })
    ...
    # ── Cron API (GET) ──
    # All cron handlers touch cron.jobs which resolves EBURON_HOME from
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/kanban/"):
        ...
        return True
    if parsed.path == "/api/tts/speak":
        return _handle_tts_speak(handler, body)
    if parsed.path == "/api/dashboard/config":
        from api import dashboard_probe
    ...
    if parsed.path == "/api/settings":
        ...
        # #1560: EBURON_HUB_PASSWORD env var takes precedence in
        ...
        if requested_password or requested_clear_password:
            if os.getenv("EBURON_HUB_PASSWORD", "").strip():
                return bad(
                    handler,
                    "EBURON_HUB_PASSWORD env var is set — it overrides the settings password. "
    ...
    if parsed.path == "/api/onboarding/oauth/start":
        ...
        import os as _os
        if not is_auth_enabled() and not _os.getenv("EBURON_HUB_ONBOARDING_OPEN"):
            import ipaddress
            try:
                ...
                is_local = False
            if not is_local:
                return bad(handler, "Onboarding OAuth is only available from local networks when auth is not enabled. To bypass this on a remote server, set EBURON_HUB_ONBOARDING_OPEN=1.", 403)
        try:
    ...
    if parsed.path == "/api/onboarding/setup":
        ...
        # EBURON_HUB_ONBOARDING_OPEN=1 lets operators on remote servers explicitly bypass
        ...
        if not is_auth_enabled() and not _os.getenv("EBURON_HUB_ONBOARDING_OPEN"):
            ...
            try:
                ...
                is_local = False
            if not is_local:
                return bad(handler, "Onboarding setup is only available from local networks when auth is not enabled. To bypass this on a remote server, set EBURON_HUB_ONBOARDING_OPEN=1.", 403)
        try:
    ...
    if parsed.path == "/api/onboarding/probe":
        ...
        if not is_auth_enabled() and not _os.getenv("EBURON_HUB_ONBOARDING_OPEN"):
            ...
            try:
                ...
                is_local = False
            if not is_local:
                return bad(handler, "Onboarding probe is only available from local networks when auth is not enabled. To bypass this on a remote server, set EBURON_HUB_ONBOARDING_OPEN=1.", 403)
...
def _serve_voice_orb(handler, parsed):
    """Serve files from the voice-orb directory."""
    orb_root = (Path(__file__).parent.parent / "voice-orb").resolve()
    # If requesting the root, serve index.html
    if parsed.path in ("/voice-orb", "/voice-orb/", "/voice-orb/index.html"):
        target = orb_root / "index.html"
    else:
        rel = parsed.path[len("/voice-orb/"):]
        target = (orb_root / rel).resolve()
    try:
        target.relative_to(orb_root)
    except ValueError:
        return j(handler, {"error": "not found"}, status=404)
    if not target.exists() or not target.is_file():
        return j(handler, {"error": "not found"}, status=404)
...
def _handle_tts_speak(handler, body):
    """Generate a WAV through Kokoro or LuxTTS based on engine selection."""
    from api.luxtts import VoiceboxLuxTTSError
    from api import kokoro

    if not str(body.get("text") or "").strip():
        return bad(handler, "text is required")

    engine = str(body.get("engine") or "kokoro").strip().lower()

    try:
        if engine == "luxtts":
            from api.luxtts import synthesize_wav

            audio, meta = synthesize_wav(
                body.get("text"),
            ...
            handler.send_header("Cache-Control", "no-store")
            handler.send_header("X-TTS-Engine", str(meta.get("engine") or "luxtts"))
            handler.send_header("X-TTS-Voice", str(meta.get("profile_id") or ""))
            handler.send_header("X-TTS-Language", str(meta.get("language") or "en"))
            _security_headers(handler)
            handler.end_headers()
            ...
            # kokoro (default)
            audio, meta = kokoro.synthesize_wav(
                body.get("text"),
            ...
            handler.send_header("Content-Length", str(len(audio)))
            handler.send_header("Cache-Control", "no-store")
            handler.send_header("X-TTS-Engine", str(meta.get("engine") or "kokoro"))
            handler.send_header("X-TTS-Voice", str(meta.get("voice_id") or ""))
            handler.send_header("X-TTS-Language", str(meta.get("lang_code") or ""))
            _security_headers(handler)
            handler.end_headers()
            handler.wfile.write(audio)
            return True
    ...
    except kokoro.KokoroTTSError as exc:
        logger.warning("Kokoro request failed: %s", exc)
        return bad(handler, str(exc), status=503)
    except Exception as exc:
        logger.exception("TTS handler failed")
        return bad(handler, "TTS generation failed", status=500)
...
def _handle_media(handler, parsed):
    ...
    _EBURON_HOME = Path(_os.getenv("EBURON_HOME", str(_HOME / ".hermes"))).expanduser()
...
def _handle_chat_start(handler, body):
    ...
    try:
        from api.luxtts import normalize_luxtts_options

        tts_options = normalize_luxtts_options(body.get("tts"))
    except Exception:
        logger.debug("Invalid LuxTTS options ignored", exc_info=True)
        tts_options = None
    ...
    thr = threading.Thread(
        ...
        kwargs={"model_provider": model_provider, "tts_options": tts_options},
        daemon=True,
    )
    thr.start()
    response = {
...
def _handle_cron_run(handler, body):
    ...
    # run unpinned against the process-global EBURON_HOME — so we'd
