...
def _aiagent_import_error_detail() -> str:
    ...
    agent_dir = _os.environ.get("EBURON_HUB_AGENT_DIR")
    if agent_dir:
        lines.append(f"  EBURON_HUB_AGENT_DIR: {agent_dir}")
    else:
        lines.append("  EBURON_HUB_AGENT_DIR: (not set)")
...
def _run_agent_streaming(
    ...
    *,
    ...
    _tts_bridge = None
    if tts_options and tts_options.enabled:
        engine = getattr(tts_options, "engine", "kokoro")
        try:
            if engine == "luxtts":
                from api.luxtts import RealtimeLuxTTSBridge

                _tts_bridge = RealtimeLuxTTSBridge(
                    put,
                ...
                # kokoro (default)
                from api.kokoro import RealtimeKokoroBridge

                _tts_bridge = RealtimeKokoroBridge(
                    put,
                    session_id=session_id,
                    stream_id=stream_id,
                    options=tts_options,
            ...
            logger.debug("TTS bridge unavailable for engine %s", engine, exc_info=True)
    ...
    try:
        ...
        try:
            ...
            _profile_home = os.environ.get('EBURON_HOME', '')
        ...
        with _ENV_LOCK:
            ...
            old_hermes_home = os.environ.get('EBURON_HOME')
            os.environ.update(_profile_runtime_env)
            os.environ['TERMINAL_CWD'] = str(s.workspace)
            os.environ['HERMES_EXEC_ASK'] = '1'
            os.environ['HERMES_SESSION_KEY'] = session_id
            if _profile_home:
                os.environ['EBURON_HOME'] = _profile_home
        ...
        try:
            ...
            # Global TTS optimization — applies to ALL profiles, all sessions.
            # The agent should write every response as if it will be read aloud
            # by a natural text-to-speech voice. Short sentences, human cadence,
            # contractions, and a warm conversational tone — never robotic precision.
            _tts_optimized_prompt = (
                "[TTS Optimization] All your responses will be read aloud by a "
                "text-to-speech engine. Write in a natural, conversational tone — "
                "as if speaking face-to-face with the user. Use short sentences, "
                "natural pauses, and contractions. Prefer common expressions and "
                "a warm, human cadence. Avoid overly formal, stiff, or robotic "
                "language. Structure ideas clearly so they sound natural when spoken. "
                "EXCEPTION: Do not apply this to code blocks, filenames, technical "
                "identifiers, or any content explicitly marked as code — those must "
                "remain exact and precise as they will be read visually, not aloud.\n"
            )
            workspace_ctx = f"[Workspace: {s.workspace}]\n"
            workspace_system_msg = (
                f"{_tts_optimized_prompt}"
            ...
            with _ENV_LOCK:
                ...
                if old_hermes_home is None: os.environ.pop('EBURON_HOME', None)
                else: os.environ['EBURON_HOME'] = old_hermes_home
