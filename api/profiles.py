...
EBURON_HOME directory is used for config, skills, memory, cron, and API keys.
Profile switches update os.environ['EBURON_HOME'] and monkey-patch module-level
cached paths in hermes-agent modules (skills_tool, cron/jobs) that snapshot
EBURON_HOME at import time.
"""
import json
import logging
import os
...
def _resolve_base_hermes_home() -> Path:
    ...
    This is intentionally distinct from EBURON_HOME, which tracks the *active
    ...
    Resolution order:
      ...
      2. EBURON_HOME env var — but only if it does NOT look like a profile subdir
         (i.e. its parent is not named 'profiles').  This handles test isolation
         where EBURON_HOME is set to an isolated test state dir.
      3. ~/.hermes (always-correct default)

    The bug this prevents: if EBURON_HOME has already been mutated to
    /home/user/.hermes/profiles/webui (by init_profile_state at startup),
    reading it here would make _DEFAULT_EBURON_HOME point to that subdir,
    ...
    hermes_home = os.getenv('EBURON_HOME', '').strip()
    if hermes_home:
        p = Path(hermes_home).expanduser()
        # If EBURON_HOME points to a profiles/ subdir, walk up two levels to the base
        return _unwrap_profile_home_to_base(p)

    return Path.home() / '.hermes'

_DEFAULT_EBURON_HOME = _resolve_base_hermes_home()


def _read_active_profile_file() -> str:
    """Read the sticky active profile from ~/.hermes/active_profile."""
    ap_file = _DEFAULT_EBURON_HOME / 'active_profile'
    if ap_file.exists():
        try:
...
def _resolve_profile_home_for_name(name: str) -> Path:
    ...
    Root/default aliases resolve to _DEFAULT_EBURON_HOME.  Valid named profiles
    resolve to _DEFAULT_EBURON_HOME/profiles/<name> even when the directory has
...
def get_active_hermes_home() -> Path:
    """Return the EBURON_HOME path for the currently active profile.
...
# `cron.jobs` reads EBURON_HOME from os.environ (process-global) at function-
...
# This context manager swaps EBURON_HOME (and the cached module-level constants
...
# all webui access to EBURON_HOME goes through one thread at a time. Any
...
# (own-profile) EBURON_HOME, never a half-swapped state.
...
def _home_for_scheduled_cron_job(job: dict) -> Path:
    ...
    Jobs pinned to a named profile execute under that profile's EBURON_HOME, so
...
def install_cron_scheduler_profile_isolation() -> None:
    ...
    EBURON_HOME isolation as the manual /api/crons/run path.
    """
    try:
...
class cron_profile_context_for_home:
    """Context manager that pins EBURON_HOME to an explicit profile home path.
    ...
    def __enter__(self):
        ...
        try:
            self._prev_env = os.environ.get('EBURON_HOME')
            os.environ['EBURON_HOME'] = str(self._home)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        try:
            if self._prev_env is None:
                os.environ.pop('EBURON_HOME', None)
            else:
                os.environ['EBURON_HOME'] = self._prev_env
...
class cron_profile_context:
    """Context manager that pins EBURON_HOME to the TLS-active profile.
    ...
    def __enter__(self):
        ...
        try:
            ...
            os.environ['EBURON_HOME'] = str(home)
...
def get_hermes_home_for_profile(name: str) -> Path:
    """Return the EBURON_HOME Path for *name* without mutating any process state.
    ...
    Falls back to _DEFAULT_EBURON_HOME (same as 'default') when *name* is None,
...
def _set_hermes_home(home: Path):
    """Set EBURON_HOME env var and monkey-patch cached module-level paths."""
    os.environ['EBURON_HOME'] = str(home)

    # Patch skills_tool module-level cache (snapshots EBURON_HOME at import)
    try:
        import tools.skills_tool as _sk
        _sk.EBURON_HOME = home
...
def init_profile_state() -> None:
    ...
    Reads ~/.hermes/active_profile, sets EBURON_HOME env var, patches
...
def switch_profile(name: str, *, process_wide: bool = True) -> dict:
    ...
    # Process-wide profile switches mutate EBURON_HOME, module-level path caches,
...
def _default_profile_dict() -> dict:
    ...
    return {
        'name': 'default',
        'path': str(_DEFAULT_EBURON_HOME),
        'is_default': True,
        'is_active': True,
        'gateway_running': False,
        'model': None,
        'provider': None,
        'has_env': (_DEFAULT_EBURON_HOME / '.env').exists(),
        'skill_count': 0,
    }
...
def _profiles_root() -> Path:
    ...
    return (_DEFAULT_EBURON_HOME / 'profiles').resolve()
...
def _create_profile_fallback(name: str, clone_from: str = None,
    ...
    profile_dir = _DEFAULT_EBURON_HOME / 'profiles' / name
    ...
    if clone_config and clone_from:
        if _is_root_profile(clone_from):
            source_dir = _DEFAULT_EBURON_HOME
        else:
            source_dir = _DEFAULT_EBURON_HOME / 'profiles' / clone_from
...
def create_profile_api(name: str, clone_from: str = None,
    ...
    profile_path = _DEFAULT_EBURON_HOME / 'profiles' / name
