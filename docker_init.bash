...
# reasoning: needed when using docker compose as the file will exist in the stopped container, and changing the value from environment variables or configuration file must be propagated from eburonhubtoo to eburonhubtoo transition (those values are the only ones loaded before the environment variables dump file are loaded)
it=$itdir/eburonhub_user_uid
...
# In two-container setups (beatrice-agent + hermes-webui), the shared eburon-home
# volume may be owned by the agent container's UID — detect from there first.
if [ -z "${WANTED_UID+x}" ] || [ "${WANTED_UID}" = "1024" ]; then
  # Priority 1: eburon-home shared volume — covers two-container Zeabur/Compose setups (#668)
  for _probe_dir in "/home/eburonhub/.hermes" "$HERMES_HOME" "/opt/data"; do
...
if [ -z "${WANTED_GID+x}" ] || [ "${WANTED_GID}" = "1024" ]; then
  # Priority 1: eburon-home shared volume
  for _probe_dir in "/home/eburonhub/.hermes" "$HERMES_HOME" "/opt/data"; do
    if [ -d "$_probe_dir" ]; then
...
# eburonhubtoo is a specfiic user not existing by default on ubuntu, we can check its whomai
if [ "A${whoami}" == "Aeburonhubtoo" ]; then 
  echo "-- Running as eburonhubtoo, will switch hermeswebui to the desired UID/GID"
  # The script is started as eburonhubtoo -- UID/GID 1025/1025
  ...
  # The script runs as eburonhubtoo (non-root), but groupmod/usermod use sudo.
  ...
  sudo chown -R ${WANTED_UID}:${WANTED_GID} /home/eburonhub || error_exit "Failed to set owner of /home/eburonhub"
  save_env /tmp/eburonhubtoo_env.txt  
...
# If we are here, the script is started as another user than eburonhubtoo
...
# Load environment variables one by one if they do not exist from /tmp/eburonhubtoo_env.txt
it=/tmp/eburonhubtoo_env.txt
if [ -f $it ]; then
...
export PATH="/home/eburonhub/.local/bin/:$PATH"
...
if [ -f /app/venv/.deps_installed ]; then
  ...
  echo ""; echo "== Adding beatrice-agent's pyproject.toml base dependencies to the virtual environment"
  _agent_paths=(
    "/home/eburonhub/.hermes/beatrice-agent"
    "/opt/hermes"
  )
  _agent_src=""
  ...
  if [ -n "$_agent_src" ]; then
    uv pip install "$_agent_src[all]" --trusted-host pypi.org --trusted-host files.pythonhosted.org || error_exit "Failed to install beatrice-agent's requirements"
  else
    echo ""
    echo "!! WARNING: beatrice-agent source not found."
    ...
    echo "!!   -v /path/to/beatrice-agent:/home/eburonhub/.hermes/beatrice-agent"
