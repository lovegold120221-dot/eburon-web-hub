...
> Web companion to the Beatrice Agent CLI. Same workflows, browser-native.
>
...
| Surface | Status |
|---|---|
| **Eburon Hub CLI parity** | ✅ Complete — every CLI workflow has a web equivalent |
...
- [x] Auto-install gate (opt-in via `EBURON_WEBUI_AUTO_INSTALL=1`)
...
- [x] Eburon Hub Control Center modal (centralized actions)
...
- [x] Subpath mount support (reverse proxy at `/eburon/`)
- [x] PWA installable from any browser
- [x] Native macOS app — universal Intel + Apple Silicon, signed + notarized DMG, Sparkle 2 auto-update — see `eburon-webui/eburon-swift-mac` repo
...
| Beatrice Agent dashboard cross-link | #1459 | Detect a running Beatrice Agent and surface link in nav |
...
- **Native MCP server expose** — Eburon Hub as an MCP server for direct agent integration (#733)
- **Teams / agents management panel** — editable names, roles, assignments (#719)
- **Web UI profile model alignment with Eburon Hub runtime** — design parity (#749)
...
- Plugin marketplace — Eburon Hub skills cover this surface
